...
Orchestration complète des stratégies de trading
...
- StrategySelector : Sélecteur intelligent de stratégies

WORKFLOW PRINCIPAL :
MarketData → SignalGenerator → FinalSignal → Execution

Le SignalGenerator est le point d'entrée principal pour générer
des signaux de trading en orchestrant :
1. Bataille Navale (votre méthode signature)
2. Feature Calculator (8 features + confluence)
3. Market Regime Detector (trend vs range)
4. Trend/Range Strategies selon régime
...
# 🧠 CERVEAU CENTRAL - Point d'entrée principal
...
def create_complete_signal_system(config=None):
    """
    🎯 FACTORY PRINCIPALE - Système complet de signaux
    
    Crée un système complet prêt à l'emploi :
    - SignalGenerator configuré
    - Toutes les stratégies intégrées
    - Prêt pour génération signaux temps réel
    """
    return create_signal_generator(config)

def analyze_trading_opportunity(market_data, **kwargs):
    """
    🚀 POINT D'ENTRÉE PRINCIPAL - Analyse opportunité complète
...
# Usage simplifié pour utilisateurs
def get_signal_now(market_data, **data_sources):
    """
    🎯 USAGE ULTRA SIMPLIFIÉ
    
    Point d'entrée en 1 ligne pour obtenir un signal :
