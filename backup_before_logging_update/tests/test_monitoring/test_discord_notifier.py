...
class TestDiscordIntegration:
    ...
    async def test_all_notifications(self):
        ...
        if not self.has_webhooks:
            logger.warning("❌ Pas de webhooks configurés - Skipping integration tests")
            logger.info("💡 Configurez les variables d'environnement DISCORD_WEBHOOK_* pour tester")
            return

        logger.info("🚀 Début tests intégration Discord avec vrais webhooks")
        ...
        logger.info("✅ Tests intégration Discord terminés!")
...
async def interactive_test():
    ...
    if not webhook:
        ...
        logger.info("💡 Configurez DISCORD_WEBHOOK_SIGNALS dans votre .env")
        return
    ...
    while True:
        ...
        if choice == '0':
            ...
            logger.info("🚀 Test performance - 10 messages...")
            start = datetime.now()
            for i in range(10):
                await notifier.send_custom_message(
                    NotificationChannel.SIGNALS,
