...
- NOUVEAU : Feature Calculator optimisé avec cache LRU
...
# Features - MISE À JOUR pour version optimisée
from features import create_feature_calculator  # Factory qui utilise la version optimisée par défaut
...
class MIAAutomationBot:
    ...
    - NOUVEAU : Cache optimisé pour features
    """
    ...
    def __init__(self, config: AutomationConfig):
        ...
        # Configuration cache pour optimisation
        cache_config = {
            'cache_ttl': 60,     # 1 minute
            'cache_size': 500    # 500 entrées max
        }
        
        # Créer SignalGenerator avec config cache
        signal_generator_config = {
            'cache_config': cache_config,
            'min_signal_confidence': self.config.trading.battle_navale_min_confidence,
            'min_confluence_score': self.config.trading.min_confluence_score
        }
        
        self.signal_generator = SignalGenerator(signal_generator_config)
        
        # Le SignalGenerator utilise maintenant automatiquement le FeatureCalculator optimisé
        logger.info("✅ SignalGenerator créé avec FeatureCalculator optimisé")
    ...
    # === NOUVEAU : MÉTHODE POUR MONITORER LE CACHE ===
    
    async def _log_cache_performance(self):
        """Log les performances du cache périodiquement"""
        try:
            ...
            if cache_perf:
                logger.info(f"📊 Cache Performance: "
                           f"Hit rate={cache_perf.get('hit_rate', 0):.1%}, "
                           f"Avg time={cache_perf.get('avg_calc_time_ms', 0):.2f}ms, "
                           f"Min={cache_perf.get('min_calc_time_ms', 0):.2f}ms, "
                           f"Max={cache_perf.get('max_calc_time_ms', 0):.2f}ms")
        except Exception as e:
            logger.debug(f"Erreur log cache: {e}")
    
    async def _log_cache_statistics(self):
        """Log les statistiques du cache périodiquement"""
        try:
            if hasattr(self.signal_generator, 'feature_calculator'):
                calc = self.signal_generator.feature_calculator
                if hasattr(calc, 'get_cache_stats'):
                    ...
                    self.stats['cache_hit_rate'] = stats.get('hit_rate', 0)
                    
                    # Logger
                    logger.info(f"📊 Cache Stats - Hit rate: {stats.get('hit_rate', 0):.1%}, "
                               f"Hits: {stats.get('cache_hits', 0)}, "
                               f"Misses: {stats.get('cache_misses', 0)}, "
                               f"Avg time: {stats.get('avg_calculation_time_ms', 0):.2f}ms")
                    
                    # Alert si performance dégradée
                    if stats.get('avg_calculation_time_ms', 0) > 10:
                        logger.warning(f"⚠️ Performance feature calculator dégradée: "
                                     f"{stats.get('avg_calculation_time_ms', 0):.1f}ms")
        except Exception as e:
            logger.debug(f"Erreur log cache stats: {e}")
    ...
    async def _validate_system(self) -> bool:
        ...
        # NOUVEAU : 5. Cache feature calculator
        cache_ok = self._validate_cache()
        validations.append(("Cache optimisé", cache_ok))
    ...
    def _validate_cache(self) -> bool:
        """Valide que le cache est fonctionnel"""
        try:
            if hasattr(self.signal_generator, 'feature_calculator'):
                calc = self.signal_generator.feature_calculator
                return hasattr(calc, 'get_cache_stats')
            return False
        except:
            return False
...
⚡ Cache: Activé (TTL 60s, Max 500 entrées)
        """
    ...
    async def _run_main_loop(self):
        ...
        last_cache_log = time.time()  # NOUVEAU : Timer pour log cache
        
        # Variable pour tracking
        if not hasattr(self, 'last_cache_log'):
            self.last_cache_log = time.time()
        ...
        while self.is_running and not self.shutdown_requested:
            try:
                ...
                # 3. Log cache stats toutes les 5 minutes
                if time.time() - self.last_cache_log > 300:
                    await self._log_cache_performance()
                    self.last_cache_log = time.time()
                
                # 4. Check arrêt demandé
    ...
    async def _perform_health_check(self):
        ...
        try:
            ...
            stats = self.signal_generator.get_performance_stats()
            cache_perf = stats.get('cache_performance', {})
            if cache_perf.get('hit_rate', 0) < 0.3:  # Moins de 30% de hit rate
                logger.warning("⚠️ Cache hit rate faible : {:.1%}".format(cache_perf.get('hit_rate', 0)))
    ...
    async def _update_performance_metrics(self):
        ...
        try:
            ...
            # NOUVEAU : Ajouter cache hit rate
            if self.stats['cache_hit_rate'] > 0:
                logger.info(f"⚡ Cache Performance: Hit rate {self.stats['cache_hit_rate']:.1%}")
    ...
    async def _save_final_data(self):
        ...
        try:
            ...
            # Sauvegarde stats (incluant cache)
