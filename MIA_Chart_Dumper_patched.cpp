...
SCDLLName("MIA_Chart_Dumper_Patched_VIX_Daily")

// Dumper complet : BaseData, DOM live, VAP, T&S, VWAP + VVA (Volume Value Area Lines) + PVWAP + VIX
// Collecte VAH/VAL/VPOC (courant) + PVAH/PVAL/PPOC (précédent) + PVWAP (VWAP période précédente) + VIX temps réel
...
SCSFExport scsf_MIA_Chart_Dumper_Patched_VIX_Daily(SCStudyInterfaceRef sc)
{
  if (sc.SetDefaults)
  {
    sc.GraphName = "MIA Chart Dumper (Patched + VIX Daily)";
    sc.AutoLoop = 0;
    sc.UpdateAlways = 1;
