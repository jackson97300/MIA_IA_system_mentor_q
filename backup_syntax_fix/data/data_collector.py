...
class DataCollector:
    ...
    def _prepare_daily_ml_features(self, target_date: date, trades_data: Dict):
        ...
        try:
            ...
            if features_data:
                ...
                logger.debug(f"Features ML sauvegardées: {target_date} ({len(features_data)} trades)")
    ...
    def export_ml_training_dataset(self, 
        ...
        try:
            ...
            features_df, labels_series, metadata_df = self._prepare_ml_features_and_labels(all_trades_data)
            ...
            logger.info(f"Dataset ML créé: {len(features_df)} samples, {len(features_df.columns)} features")
            return ml_dataset
    ...
    def _prepare_ml_features_and_labels(self, trades_data: Dict) -> Tuple[pd.DataFrame, pd.Series, pd.DataFrame]:
        ...
        for trade_id, trade_snapshots in trades_data.items():
            ...
            # Extraction features
            trade_features = self._extract_comprehensive_features(decision_snapshot, result_snapshot)
    ...
    def _extract_comprehensive_features(self, decision_snapshot: Dict, result_snapshot: Dict) -> Optional[Dict]:
