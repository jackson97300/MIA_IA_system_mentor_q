#!/usr/bin/env python3
"""
🧪 TEST DEMO MODE VALIDATION - MIA_IA_SYSTEM
Test spécifique pour valider l'objectif 25-30% de valid rate
- Mode demo activé
- Seuils adaptés
- 60 itérations
...
def test_demo_mode_validation():
    """Test principal pour valider l'objectif 25-30%"""
    logger.info("🎯 TEST DEMO MODE VALIDATION - Objectif 25-30%")
    logger.info("=" * 60)
    
    # 1. Configuration demo ULTRA-permissive
    demo_calibration = LeadershipCalibration(
        corr_min=0.15,  # Seuil ultra-permissif
        leader_strength_min=0.10,  # Ultra-permissif
    ...
    config_manager.calibration = demo_calibration  # Override direct
    ...
    logger.info("⚙️ Configuration Demo ULTRA-PERMISSIVE:")
    logger.info(f"  📈 Corr min: {demo_calibration.corr_min}")
    logger.info(f"  💪 Leader strength min: {demo_calibration.leader_strength_min}")
    logger.info(f"  ⏱️ Persistence bars: {demo_calibration.persistence_bars}")
    ...
    if valid_rate < 25:
        ...
        logger.info("    - Réduire corr_min (actuel: 0.15)")
        logger.info("    - Réduire leader_strength_min (actuel: 0.10)")
