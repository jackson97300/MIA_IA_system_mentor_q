...
class MLDataProcessor:
    ...
    def clean_snapshot_data(self, raw_data: List[Dict]) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(f"Nettoyage terminé: {initial_size} → {final_size} samples ({retention_rate:.1f}% rétention)")
    ...
    def create_ml_dataset(self, 
        ...
        try:
            ...
            logger.info(f"Dataset final: Train={len(X_train)}, Test={len(X_test)}, Features={len(feature_columns)}")
    ...
    def _create_interaction_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'bn_confluence_battle' in df.columns and 'ms_volume_profile_strength' in df.columns:
            df['interaction_confluence_volume'] = df['bn_confluence_battle'] * df['ms_volume_profile_strength']
    ...
    def _time_series_split(self, data: pd.DataFrame, target_col: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    def _stratified_split(self, data: pd.DataFrame, target_col: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """Split stratifié pour équilibrer les classes"""
    ...
    def _random_split(self, data: pd.DataFrame, target_col: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """Split aléatoire standard"""
    ...
    def _walk_forward_split(self, data: pd.DataFrame, target_col: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        for export_format in self.config.export_formats:
            try:
                if export_format == "pickle":
                    ...
                    dataset.X_train.to_csv(self.output_dir / f"X_train_{timestamp}.csv", index=False)
                    dataset.X_test.to_csv(self.output_dir / f"X_test_{timestamp}.csv", index=False)
                    dataset.y_train.to_csv(self.output_dir / f"y_train_{timestamp}.csv", index=False)
                    dataset.y_test.to_csv(self.output_dir / f"y_test_{timestamp}.csv", index=False)
                
                elif export_format == "parquet":
                    # Export optimisé Parquet
                    dataset.X_train.to_parquet(self.output_dir / f"X_train_{timestamp}.parquet", index=False)
                    dataset.X_test.to_parquet(self.output_dir / f"X_test_{timestamp}.parquet", index=False)
