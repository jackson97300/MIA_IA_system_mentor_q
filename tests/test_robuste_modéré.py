...
config_data = {
    "name": "MODÉRÉ_ROBUSTE",
    "description": "Win Rate 45-55% - Paramètres équilibrés (Version robuste)",
...
logging.basicConfig(
    level=logging.INFO, 
    format='%(asctime)s - MODÉRÉ_ROBUSTE - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler(f'logs/test_robuste_modéré.log', encoding='utf-8'),
...
async def main():
    """Lancement test MODÉRÉ ROBUSTE"""
    
    logger.info("🚀 === TEST MODÉRÉ ROBUSTE DÉMARRÉ ===")
    ...
    while retry_count < max_retries:
        try:
            ...
            # Appliquer paramètres de test MODÉRÉ
            ...
            logger.info("⏹️ Test MODÉRÉ ROBUSTE arrêté par l'utilisateur")
            break
            
        except Exception as e:
            retry_count += 1
            logger.error(f"❌ Erreur test MODÉRÉ ROBUSTE (tentative {retry_count}): {e}")
    ...
    logger.info("🏁 === TEST MODÉRÉ ROBUSTE TERMINÉ ===")
