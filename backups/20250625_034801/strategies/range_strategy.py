...
class RangeStrategy:
    ...
    def _detect_range_structure(self,
        ...
        resistance_level = self._find_clustered_level(highs, resistance_zone, tolerance=2*ES_TICK_SIZE)
        ...
        resistance_tests = self._count_level_tests(highs, resistance_level, tolerance=1.5*ES_TICK_SIZE)
    ...
    def _analyze_bounce_opportunity(self,
        ...
        if distance_to_support <= self.proximity_tolerance * ES_TICK_SIZE:
            ...
            pattern_confirmed, pattern_name, pattern_strength = self._check_bullish_patterns(sierra_patterns)
        ...
        # BOUNCE RÉSISTANCE (SHORT)
            ...
            pattern_confirmed, pattern_name, pattern_strength = self._check_bearish_patterns(sierra_patterns)
            volume_confirmed = self._check_volume_confirmation(market_data, "resistance")
            confluence = self._calculate_bounce_confluence(current_price, structure_data, "resistance")
    ...
    def _generate_range_signal(self,
        ...
        if bounce_type == "support":
            direction = "LONG"
            entry_price = bounce_setup.target_level + (0.5 * ES_TICK_SIZE)  # Légèrement au-dessus support
            target_level = range_structure.resistance_level
        else:  # resistance
            direction = "SHORT"
            entry_price = bounce_setup.target_level - (0.5 * ES_TICK_SIZE)  # Légèrement sous résistance
    ...
    def _calculate_adaptive_stop(self,
        ...
        if base_size >= self.wide_stop_threshold:
            ...
            logger.info(f"Stop élargi appliqué: base {base_size:.1f} ticks → stop {stop_distance_ticks:.1f} ticks")
        else:
            # Stop normal
    ...
    def _check_bullish_patterns(self, sierra_patterns: Optional[Dict[str, float]]) -> Tuple[bool, str, float]:
    ...
    def _check_bearish_patterns(self, sierra_patterns: Optional[Dict[str, float]]) -> Tuple[bool, str, float]:
