...
1. 🔌 CONNEXION MULTI-BROKER - IBKR primary + Sierra Chart backup
2. 📊 MODES ADAPTATIFS - Simulation/Paper/Live selon contexte
3. ✅ VALIDATION ORDRES - Prix, taille, limites avant soumission
4. 🔄 RETRY LOGIC - Reconnexion auto, retry intelligent
5. 📈 MONITORING - Status ordres, fills, rejets temps réel
6. 🛡️ SÉCURITÉ - Limites position, validation compte
7. 🔧 INTÉGRATION - Configuration Sierra Chart + IBKR centralisée

WORKFLOW PRINCIPAL :
SimpleBattleNavaleTrader → OrderManager → IBKR/Sierra → Fills → Position Updates

MODES SUPPORTÉS :
- SIMULATION : Ordres simulés pour DATA_COLLECTION et tests
...
try:
    ...
    from config.sierra_config import (
        get_sierra_config, 
        SierraIBKRConfig, 
        BrokerType as SierraBrokerType,
        OrderProvider,
        DataProvider
    )
    ...
    def get_automation_config():
        return {"enable_ibkr": True, "enable_sierra": True}
    def get_sierra_config():
        return None
...
class TradingMode(Enum):
    """Modes de trading"""
    SIMULATION = "simulation"    # Simulation complète (data collection)
    PAPER = "paper"             # Paper trading broker réel
...
class OrderManager:
    ...
    def __init__(self, mode: str = "simulation", sierra_config: Optional[SierraIBKRConfig] = None):
        """
        ...
        self.sierra_config = sierra_config or (get_sierra_config() if CONFIG_AVAILABLE else None)
        self.config = get_automation_config() if CONFIG_AVAILABLE else {}
        ...
        logger.info(f"OrderManager initialisé en mode {mode}")
        logger.info(f"  - Trading Mode: {self.mode.value}")
        logger.info(f"  - Order Provider: {self.order_provider.value if hasattr(self.order_provider, 'value') else self.order_provider}")
        
        # Auto-connect selon le mode et configuration
        if self.mode == TradingMode.SIMULATION or self.order_provider == OrderProvider.SIMULATION:
    ...
    def _init_broker_connections(self):
        ...
        try:
            ...
            if self.data_provider.value == "ibkr" or self.order_provider == OrderProvider.IBKR:
                ...
                self.connections[BrokerType.IBKR] = BrokerConnection(
                    ...
                    trading_enabled=self.order_provider == OrderProvider.IBKR
                )
                logger.info(f"IBKR configuré: {ibkr_config.host}:{ibkr_config.port} (client {ibkr_config.client_id})")
            
            # Sierra Chart connection selon configuration
            if self.order_provider == OrderProvider.SIERRA_CHART:
                ...
                logger.info(f"Sierra Chart configuré: {sierra_config.server_address}:{sierra_config.server_port}")
    ...
    async def connect(self) -> bool:
        ...
        if self.kill_switch_triggered:
            logger.error("❌ Kill switch activé, connexion interdite")
            return False
        ...
        if not success and self.backup_broker in self.connections:
            logger.warning(f"Échec {self.primary_broker.value}, tentative {self.backup_broker.value}")
        ...
        if success:
            logger.info(f"✅ Connecté au broker {self.current_broker.value}")
        else:
            logger.error(f"❌ Échec connexion tous brokers")
        ...
        try:
            ...
            if client.isConnected():
                ...
                if self.mode == TradingMode.LIVE and connection.port == 7497:
                    logger.warning("⚠️ Mode LIVE mais port paper (7497) détecté")
                elif self.mode == TradingMode.PAPER and connection.port == 7496:
                    logger.warning("⚠️ Mode PAPER mais port live (7496) détecté")
                
                logger.info(f"✅ IBKR connecté sur {connection.host}:{connection.port}")
                return True
    ...
    async def _connect_sierra(self, connection: BrokerConnection) -> bool:
        ...
        try:
            ...
            # TODO: Implement Sierra Chart DTC protocol
            logger.info(f"Sierra Chart connexion simulée sur {connection.host}:{connection.port}")
            
            # Pour l'instant, simuler connexion réussie
            connection.status = ConnectionStatus.AUTHENTICATED
            connection.connection_count += 1
            connection.last_heartbeat = datetime.now()
            
            logger.info(f"✅ Sierra Chart connecté (simulé)")
            return True
    ...
    async def submit_order(self, order_details: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if (self.mode == TradingMode.SIMULATION or 
                self.order_provider == OrderProvider.SIMULATION):
        ...
        try:
            ...
            if self.sierra_config:
                ...
                if current_connection and order.size > current_connection.max_order_size:
                    return {'valid': False, 'error': f'Taille ordre > limite ({current_connection.max_order_size})'}
                ...
                if order.size > security_config.max_single_position:
                    return {'valid': False, 'error': f'Position > limite sécurité ({security_config.max_single_position})'}
        ...
        try:
            ...
            if current_connection and hasattr(current_connection, 'daily_loss_limit'):
                if self.execution_stats['daily_pnl'] <= -current_connection.daily_loss_limit:
                    logger.warning(f"Daily loss limit atteinte: {self.execution_stats['daily_pnl']}")
                    return False
    ...
    async def _execute_ibkr_order(self, order_id: str, order: OrderRequest, 
        ...
        """Exécute ordre IBKR"""
        try:
            # Pour l'instant, utiliser simulation même en mode paper/live
            # TODO: Implémenter soumission ordre IBKR réelle avec ib_insync
            
            logger.info(f"Ordre IBKR {order_id} (simulé en attendant implémentation)")
    ...
    async def _execute_sierra_order(self, order_id: str, order: OrderRequest,
        ...
        try:
            ...
            logger.info(f"Ordre Sierra Chart {order_id} (simulé en attendant implémentation)")
            result = await self._execute_simulated_order(order_id, order)
        ...
        try:
            with self.lock:
                ...
                if symbol not in self.positions:
                    ...
                    logger.info(f"Nouvelle position: {symbol} {order.side} {result.fill_quantity} @ {result.fill_price}")
                else:
                    # Position existante
                    ...
                    if pos.side == order.side:
                        ...
                        total_value = (pos.quantity * pos.avg_price) + (result.fill_quantity * result.fill_price)
                        pos.avg_price = total_value / total_quantity
                        pos.quantity = total_quantity
                        logger.info(f"Position augmentée: {symbol} {pos.side} {pos.quantity} @ {pos.avg_price:.2f}")
                    else:
                        ...
                        if pos.quantity <= 0:
                            ...
                            logger.info(f"Position réduite: {symbol} {pos.side} {pos.quantity} @ {pos.avg_price:.2f}")
    ...
    async def _check_kill_switch(self):
        ...
        try:
            ...
            total_positions = sum(abs(pos.quantity) for pos in self.positions.values())
            if total_positions >= security_config.kill_switch_position_threshold:
    ...
    async def _trigger_kill_switch(self, reason: str):
        ...
        try:
            ...
            logger.error(f"🚨 KILL SWITCH DÉCLENCHÉ: {reason}")
            ...
            logger.error("🛑 Trading automatiquement suspendu")
    ...
    def cancel_order(self, order_id: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Annulation ordre {order_id} (placeholder)")
            return True
        ...
        try:
            ...
            for position in positions_to_close:
                ...
                if result.get('status') == 'FILLED':
                    ...
                    logger.error(f"Échec fermeture position {position.symbol}: {result.get('error_message')}")
            
            logger.info(f"✅ Fermé {len(closed_orders)} positions avec succès")
    ...
    async def disconnect(self):
        ...
        try:
            ...
            logger.info("✅ OrderManager déconnecté")
    ...
    def log_diagnostics(self):
        ...
        logger.info("=" * 60)
        logger.info("📊 DIAGNOSTICS ORDER MANAGER")
        logger.info("=" * 60)
        ...
        logger.info(f"Is Connected:              {'✅' if self.is_connected() else '❌'}")
        logger.info(f"Kill Switch:               {'🚨 ACTIVÉ' if self.kill_switch_triggered else '✅ Normal'}")
        
        # Connexions
        logger.info(f"\n🔌 CONNEXIONS:")
        for broker_type, connection in self.connections.items():
            logger.info(f"{broker_type.value:15} {connection.status.value:15} {connection.host}:{connection.port}")
...
def create_order_manager(mode: str = "simulation", sierra_config: Optional[SierraIBKRConfig] = None) -> OrderManager:
...
async def test_order_manager():
    ...
    # Créer instance
    om = create_order_manager("simulation", sierra_config)
    ...
    logger.info("Is connected: {om.is_connected()}")
    ...
    logger.info("Ordre résultat: {result}")
    ...
    logger.info("Stats: Fill rate {stats['fill_rate']:.1%}")
    
    # Test connexion status
    conn_status = om.get_connection_status()
    logger.info("Connexion status: {conn_status['current_broker']}")
    
    logger.info("\n✅ Order Manager v4.0 test completed!")
    return True
