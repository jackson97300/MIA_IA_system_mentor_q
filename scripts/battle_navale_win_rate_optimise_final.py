...
# -*- coding: utf-8 -*-
"""
Battle Navale Win Rate Optimisé Final - MIA_IA System
Version finale optimisée pour Win Rate 66.7%+ avec plus de signaux
"""
...
class BattleNavaleWinRateOptimiseFinal:
    def __init__(self):
        self.test_results = {
        ...
        # Paramètres OPTIMISÉS FINAUX pour Win Rate + Volume
        self.params = {
            'position_size': 0.07,     # 7% - Conservateur
            'take_profit': 0.018,      # 1.8% - Rapide
            'stop_loss': 0.009,        # 0.9% - Serré
            'max_positions': 6         # Plus de positions
        }
        
        # Filtres OPTIMISÉS pour plus de signaux
        self.filters = {
            'min_confluence': 0.40,    # Score confluence plus bas pour plus de signaux
            'max_daily_trades': 4,     # Max 4 trades par jour
    ...
    def load_data(self):
        ...
        print("📂 Chargement données pour Battle Navale Win Rate Optimisé Final...")
    ...
    def _calculate_technical_indicators(self):
        """Calculer indicateurs techniques optimisés finaux"""
    ...
    def _calculate_confluence_score(self, date, current_price, signal_type):
        """Calculer score de confluence optimisé final"""
        confluence_factors = {
            'technical': 0,
            'volume': 0,
            'options': 0,
        ...
        # RSI avec seuils plus permissifs
        rsi = row['RSI']
        ...
        # MACD avec confirmation plus permissive
        macd = row['MACD']
        ...
        # Bollinger Bands proximité
        bb_upper = row['BB_Upper']
        bb_lower = row['BB_Lower']
        if signal_type == 'BUY' and current_price < bb_lower * 1.02:
            tech_score += 0.3
        elif signal_type == 'SELL' and current_price > bb_upper * 0.98:
            tech_score += 0.3
        ...
        if date in self.orderflow_data:
            ...
            if signal_type == 'BUY' and flow_direction == 'BULLISH' and flow_intensity > 1.1:
                orderflow_score = 0.8
            elif signal_type == 'SELL' and flow_direction == 'BEARISH' and flow_intensity > 1.1:
                orderflow_score = 0.8
        ...
        # 7. FACTEURS SUPPORT/RESISTANCE (5%)
        sr_score = 0
        support = row['Support_20']
        resistance = row['Resistance_20']
        
        if signal_type == 'BUY' and current_price < support * 1.01:
            sr_score = 0.8
        elif signal_type == 'SELL' and current_price > resistance * 0.99:
            sr_score = 0.8
        
        confluence_factors['support_resistance'] = sr_score
        ...
        confluence_factors['total'] = (
            ...
            confluence_factors['momentum'] * 0.10 +
            confluence_factors['support_resistance'] * 0.05
        )
        
        return confluence_factors
    
    def _apply_optimized_filters(self, date, current_price, signal_type, confluence_score):
        """Appliquer filtres optimisés pour plus de signaux"""
        row = self.historical_data.loc[date]
        ...
        if self.filters['trend_filter']:
            sma_50 = row['SMA_50']
            if signal_type == 'BUY' and current_price < sma_50 * 0.95:
                return False
            elif signal_type == 'SELL' and current_price > sma_50 * 1.05:
                return False
    ...
    def _generate_optimized_final_signals(self, date, current_price):
        """Générer signaux optimisés finaux avec plus de variété"""
        signals = []
        
        row = self.historical_data.loc[date]
        
        # 1. SIGNAL RSI (Seuils permissifs)
        rsi = row['RSI']
        if rsi < 40:  # Survente permissive
            confluence = self._calculate_confluence_score(date, current_price, 'BUY')
            if self._apply_optimized_filters(date, current_price, 'BUY', confluence['total']):
                signals.append({
                    'type': 'RSI_OVERSOLD_PERMISSIVE',
                    'action': 'BUY',
                    'strength': 0.7,
                    'confluence_score': confluence['total'],
                    'reason': f'RSI survente permissive: {rsi:.1f}'
                })
        elif rsi > 60:  # Surachat permissif
            confluence = self._calculate_confluence_score(date, current_price, 'SELL')
            if self._apply_optimized_filters(date, current_price, 'SELL', confluence['total']):
                signals.append({
                    'type': 'RSI_OVERBOUGHT_PERMISSIVE',
                    'action': 'SELL',
                    'strength': 0.7,
                    'confluence_score': confluence['total'],
                    'reason': f'RSI surachat permissif: {rsi:.1f}'
                })
        
        # 2. SIGNAL MACD (Croisement simple)
        macd = row['MACD']
        macd_signal = row['MACD_Signal']
        
        if macd > macd_signal:  # Signal haussier
            confluence = self._calculate_confluence_score(date, current_price, 'BUY')
            if self._apply_optimized_filters(date, current_price, 'BUY', confluence['total']):
                signals.append({
                    'type': 'MACD_BULLISH_SIMPLE',
                    'action': 'BUY',
                    'strength': 0.6,
                    'confluence_score': confluence['total'],
                    'reason': f'MACD haussier simple: {macd:.4f}'
                })
        elif macd < macd_signal:  # Signal baissier
            confluence = self._calculate_confluence_score(date, current_price, 'SELL')
            if self._apply_optimized_filters(date, current_price, 'SELL', confluence['total']):
                signals.append({
                    'type': 'MACD_BEARISH_SIMPLE',
                    'action': 'SELL',
                    'strength': 0.6,
                    'confluence_score': confluence['total'],
                    'reason': f'MACD baissier simple: {macd:.4f}'
                })
        
        # 3. SIGNAL BOLLINGER BANDS (Proximité permissive)
        bb_upper = row['BB_Upper']
        bb_lower = row['BB_Lower']
        
        if current_price < bb_lower * 1.01:  # Proximité support
            confluence = self._calculate_confluence_score(date, current_price, 'BUY')
            if self._apply_optimized_filters(date, current_price, 'BUY', confluence['total']):
                signals.append({
                    'type': 'BB_SUPPORT_PERMISSIVE',
                    'action': 'BUY',
                    'strength': 0.8,
                    'confluence_score': confluence['total'],
                    'reason': f'Proximité support BB permissive: {bb_lower:.2f}'
                })
        elif current_price > bb_upper * 0.99:  # Proximité résistance
            confluence = self._calculate_confluence_score(date, current_price, 'SELL')
            if self._apply_optimized_filters(date, current_price, 'SELL', confluence['total']):
                signals.append({
                    'type': 'BB_RESISTANCE_PERMISSIVE',
                    'action': 'SELL',
                    'strength': 0.8,
                    'confluence_score': confluence['total'],
                    'reason': f'Proximité résistance BB permissive: {bb_upper:.2f}'
                })
        
        # 4. SIGNAL GEX (Proximité permissive)
        if date in self.options_data:
            gex1 = self.options_data[date]['gamma_exposure']['gex1']
            gex2 = self.options_data[date]['gamma_exposure']['gex2']
            
            if current_price < gex1 * 1.01:  # Proximité support GEX
                confluence = self._calculate_confluence_score(date, current_price, 'BUY')
                if self._apply_optimized_filters(date, current_price, 'BUY', confluence['total']):
                    signals.append({
                        'type': 'GEX_SUPPORT_PERMISSIVE',
                        'action': 'BUY',
                        'strength': 0.9,
                        'confluence_score': confluence['total'],
                        'reason': f'Proximité support GEX permissive: {gex1:.2f}'
                    })
            elif current_price > gex2 * 0.99:  # Proximité résistance GEX
                confluence = self._calculate_confluence_score(date, current_price, 'SELL')
                if self._apply_optimized_filters(date, current_price, 'SELL', confluence['total']):
                    signals.append({
                        'type': 'GEX_RESISTANCE_PERMISSIVE',
                        'action': 'SELL',
                        'strength': 0.9,
                        'confluence_score': confluence['total'],
                        'reason': f'Proximité résistance GEX permissive: {gex2:.2f}'
                    })
        
        # 5. SIGNAL VWAP (Nouveau - permissif)
        vwap = row['VWAP']
        vwap_distance = abs(current_price - vwap) / vwap
        
        if vwap_distance > 0.006:  # Écart plus permissif
            if current_price < vwap:
                confluence = self._calculate_confluence_score(date, current_price, 'BUY')
                if self._apply_optimized_filters(date, current_price, 'BUY', confluence['total']):
                    signals.append({
                        'type': 'VWAP_MEAN_REVERSION_PERMISSIVE',
                        'action': 'BUY',
                        'strength': 0.5,
                        'confluence_score': confluence['total'],
                        'reason': f'Retour VWAP permissif: {vwap:.2f}'
                    })
            else:
                confluence = self._calculate_confluence_score(date, current_price, 'SELL')
                if self._apply_optimized_filters(date, current_price, 'SELL', confluence['total']):
        ...
        # 6. SIGNAL STOCHASTIC (Nouveau)
        stoch_k = row['Stoch_K']
        stoch_d = row['Stoch_D']
        
        if stoch_k < 30 and stoch_d < 30:  # Survente
            confluence = self._calculate_confluence_score(date, current_price, 'BUY')
            if self._apply_optimized_filters(date, current_price, 'BUY', confluence['total']):
                signals.append({
                    'type': 'STOCH_OVERSOLD',
                    ...
                    'strength': 0.6,
                    'confluence_score': confluence['total'],
                    'reason': f'Stochastic survente: K={stoch_k:.1f}, D={stoch_d:.1f}'
                })
        elif stoch_k > 70 and stoch_d > 70:  # Surachat
            confluence = self._calculate_confluence_score(date, current_price, 'SELL')
            if self._apply_optimized_filters(date, current_price, 'SELL', confluence['total']):
                signals.append({
                    ...
                    'strength': 0.6,
                    'confluence_score': confluence['total'],
                    'reason': f'Stochastic surachat: K={stoch_k:.1f}, D={stoch_d:.1f}'
                })
        
        # 7. SIGNAL WILLIAMS %R (Nouveau)
        williams_r = row['Williams_R']
        
        if williams_r < -80:  # Survente
            confluence = self._calculate_confluence_score(date, current_price, 'BUY')
            if self._apply_optimized_filters(date, current_price, 'BUY', confluence['total']):
                signals.append({
                    ...
                    'strength': 0.6,
                    'confluence_score': confluence['total'],
                    'reason': f'Williams %R survente: {williams_r:.1f}'
                })
        elif williams_r > -20:  # Surachat
            confluence = self._calculate_confluence_score(date, current_price, 'SELL')
            if self._apply_optimized_filters(date, current_price, 'SELL', confluence['total']):
                signals.append({
                    ...
                    'strength': 0.6,
                    'confluence_score': confluence['total'],
                    'reason': f'Williams %R surachat: {williams_r:.1f}'
                })
        
        # 8. SIGNAL CCI (Nouveau)
        cci = row['CCI']
        ...
        if cci < -100:  # Survente
            ...
            if self._apply_optimized_filters(date, current_price, 'BUY', confluence['total']):
                signals.append({
                    ...
                    'strength': 0.6,
                    'confluence_score': confluence['total'],
                    'reason': f'CCI survente: {cci:.1f}'
                })
        elif cci > 100:  # Surachat
            confluence = self._calculate_confluence_score(date, current_price, 'SELL')
            if self._apply_optimized_filters(date, current_price, 'SELL', confluence['total']):
                signals.append({
                    'type': 'CCI_OVERBOUGHT',
    ...
    def run_optimized_final_backtest(self):
        """Exécuter backtest optimisé final"""
        print("⚔️ Démarrage Battle Navale Win Rate Optimisé Final...")
        ...
        print(f"   📊 Paramètres optimisés finaux appliqués")
        print(f"   🔍 Filtres optimisés activés")
        
        # Parcourir chaque jour
        for i, (date, row) in enumerate(self.historical_data.iterrows()):
            current_price = row['Close']
            
            # Générer signaux optimisés finaux
            signals = self._generate_optimized_final_signals(date, current_price)
    ...
    def _calculate_results(self, trades, equity_curve, initial_capital):
        """Calculer résultats"""
        print("📊 Calcul résultats Battle Navale Win Rate Optimisé Final...")
    ...
    def save_results(self):
        ...
        filename = f"data/test_results/battle_navale_win_rate_optimise_final_{timestamp}.json"
    ...
    def run_complete_backtest(self):
        ...
        print("⚔️ BATTLE NAVALE WIN RATE OPTIMISÉ FINAL - MIA_IA SYSTEM")
        print("=" * 60)
        ...
        # 2. Exécuter Battle Navale optimisé final
        if not self.run_optimized_final_backtest():
            return False
        ...
        # 4. Résultats finaux
        print("\n" + "=" * 60)
        print("📊 RÉSULTATS BATTLE NAVALE WIN RATE OPTIMISÉ FINAL")
        print("=" * 60)
        ...
        if (trading.get('total_trades', 0) > 25 and 
            ...
            print("\n🎉 BATTLE NAVALE WIN RATE OPTIMISÉ FINAL RÉUSSI !")
            self.test_results['success'] = True
        else:
            print("\n⚠️ Battle Navale Win Rate optimisé final - optimisations nécessaires")
...
def main():
    ...
    backtest = BattleNavaleWinRateOptimiseFinal()
    success = backtest.run_complete_backtest()
    
    if success:
        print("\n🎉 BATTLE NAVALE WIN RATE OPTIMISÉ FINAL VALIDÉ !")
    else:
        print("\n⚠️ Battle Navale Win Rate optimisé final nécessite des optimisations")
