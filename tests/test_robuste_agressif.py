...
config_data = {
    "name": "AGRESSIF_ROBUSTE",
    "description": "Win Rate 50-60% - Paramètres optimistes (Version robuste)",
...
logging.basicConfig(
    level=logging.INFO, 
    format='%(asctime)s - AGRESSIF_ROBUSTE - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler(f'logs/test_robuste_agressif.log', encoding='utf-8'),
...
async def main():
    """Lancement test AGRESSIF ROBUSTE"""
    
    logger.info("🚀 === TEST AGRESSIF ROBUSTE DÉMARRÉ ===")
    ...
    while retry_count < max_retries:
        try:
            ...
            # Appliquer paramètres de test AGRESSIF
            ...
            logger.info("⏹️ Test AGRESSIF ROBUSTE arrêté par l'utilisateur")
            break
            
        except Exception as e:
            retry_count += 1
            logger.error(f"❌ Erreur test AGRESSIF ROBUSTE (tentative {retry_count}): {e}")
    ...
    logger.info("🏁 === TEST AGRESSIF ROBUSTE TERMINÉ ===")
