...
# -*- coding: utf-8 -*-
"""
Battle Navale Équilibré Win Rate - MIA_IA System
Version équilibrée pour Win Rate 66.7%+ sans filtres trop stricts
"""
...
class BattleNavaleEquilibreWinRate:
    def __init__(self):
        self.test_results = {
        ...
        # Paramètres ÉQUILIBRÉS pour Win Rate
        self.params = {
            'position_size': 0.08,     # 8% - Équilibré
            'take_profit': 0.020,      # 2.0% - Équilibré
            'stop_loss': 0.010,        # 1.0% - Équilibré
            'max_positions': 5         # Équilibré
        }
        
        # Filtres ÉQUILIBRÉS
        self.filters = {
            'min_confluence': 0.45,    # Score confluence modéré
    ...
    def load_data(self):
        ...
        print("📂 Chargement données pour Battle Navale Équilibré Win Rate...")
    ...
    def _calculate_technical_indicators(self):
        """Calculer indicateurs techniques équilibrés"""
    ...
    def _calculate_confluence_score(self, date, current_price, signal_type):
        """Calculer score de confluence équilibré"""
        confluence_factors = {
            'technical': 0,
            'volume': 0,
            'options': 0,
        ...
        # MACD avec confirmation équilibrée
        macd = row['MACD']
    ...
    def _apply_balanced_filters(self, date, current_price, signal_type, confluence_score):
        """Appliquer filtres équilibrés"""
        row = self.historical_data.loc[date]
    ...
    def _generate_balanced_signals(self, date, current_price):
        """Générer signaux équilibrés"""
        signals = []
        
        row = self.historical_data.loc[date]
        
        # 1. SIGNAL RSI (Seuils équilibrés)
        rsi = row['RSI']
        if rsi < 35:  # Survente équilibrée
            confluence = self._calculate_confluence_score(date, current_price, 'BUY')
            if self._apply_balanced_filters(date, current_price, 'BUY', confluence['total']):
                signals.append({
                    'type': 'RSI_OVERSOLD_BALANCED',
                    'action': 'BUY',
                    'strength': 0.7,
                    'confluence_score': confluence['total'],
                    'reason': f'RSI survente équilibrée: {rsi:.1f}'
                })
        elif rsi > 65:  # Surachat équilibré
            confluence = self._calculate_confluence_score(date, current_price, 'SELL')
            if self._apply_balanced_filters(date, current_price, 'SELL', confluence['total']):
                signals.append({
                    'type': 'RSI_OVERBOUGHT_BALANCED',
                    'action': 'SELL',
                    'strength': 0.7,
                    'confluence_score': confluence['total'],
                    'reason': f'RSI surachat équilibré: {rsi:.1f}'
                })
        
        # 2. SIGNAL MACD (Croisement avec confirmation équilibrée)
        macd = row['MACD']
        macd_signal = row['MACD_Signal']
        ...
        if macd > macd_signal and macd > 0:  # Signal haussier confirmé
            ...
            if self._apply_balanced_filters(date, current_price, 'BUY', confluence['total']):
                signals.append({
                    'type': 'MACD_BULLISH_BALANCED',
                    'action': 'BUY',
                    'strength': 0.6,
                    'confluence_score': confluence['total'],
                    'reason': f'MACD haussier équilibré: {macd:.4f}'
                })
        elif macd < macd_signal and macd < 0:  # Signal baissier confirmé
            confluence = self._calculate_confluence_score(date, current_price, 'SELL')
            if self._apply_balanced_filters(date, current_price, 'SELL', confluence['total']):
                signals.append({
                    'type': 'MACD_BEARISH_BALANCED',
                    'action': 'SELL',
                    'strength': 0.6,
                    'confluence_score': confluence['total'],
                    'reason': f'MACD baissier équilibré: {macd:.4f}'
                })
        
        # 3. SIGNAL BOLLINGER BANDS (Proximité équilibrée)
        bb_upper = row['BB_Upper']
        bb_lower = row['BB_Lower']
        ...
        if current_price < bb_lower * 1.005:  # Proximité support
            ...
            if self._apply_balanced_filters(date, current_price, 'BUY', confluence['total']):
                signals.append({
                    'type': 'BB_SUPPORT_BALANCED',
                    'action': 'BUY',
                    'strength': 0.8,
                    'confluence_score': confluence['total'],
                    'reason': f'Proximité support BB équilibrée: {bb_lower:.2f}'
                })
        elif current_price > bb_upper * 0.995:  # Proximité résistance
            confluence = self._calculate_confluence_score(date, current_price, 'SELL')
            if self._apply_balanced_filters(date, current_price, 'SELL', confluence['total']):
                signals.append({
                    'type': 'BB_RESISTANCE_BALANCED',
                    'action': 'SELL',
                    'strength': 0.8,
                    'confluence_score': confluence['total'],
                    'reason': f'Proximité résistance BB équilibrée: {bb_upper:.2f}'
                })
        
        # 4. SIGNAL GEX (Proximité équilibrée)
        if date in self.options_data:
            gex1 = self.options_data[date]['gamma_exposure']['gex1']
            gex2 = self.options_data[date]['gamma_exposure']['gex2']
            ...
            if current_price < gex1 * 1.005:  # Proximité support GEX
                ...
                if self._apply_balanced_filters(date, current_price, 'BUY', confluence['total']):
                    signals.append({
                        'type': 'GEX_SUPPORT_BALANCED',
                        'action': 'BUY',
                        'strength': 0.9,
                        'confluence_score': confluence['total'],
                        'reason': f'Proximité support GEX équilibrée: {gex1:.2f}'
                    })
            elif current_price > gex2 * 0.995:  # Proximité résistance GEX
                confluence = self._calculate_confluence_score(date, current_price, 'SELL')
                if self._apply_balanced_filters(date, current_price, 'SELL', confluence['total']):
                    signals.append({
                        'type': 'GEX_RESISTANCE_BALANCED',
                        'action': 'SELL',
                        'strength': 0.9,
                        'confluence_score': confluence['total'],
                        'reason': f'Proximité résistance GEX équilibrée: {gex2:.2f}'
                    })
        
        # 5. SIGNAL VWAP (Nouveau - équilibré)
        vwap = row['VWAP']
        vwap_distance = abs(current_price - vwap) / vwap
        
        if vwap_distance > 0.008:  # Écart modéré
            if current_price < vwap:
                confluence = self._calculate_confluence_score(date, current_price, 'BUY')
                if self._apply_balanced_filters(date, current_price, 'BUY', confluence['total']):
                    signals.append({
                        'type': 'VWAP_MEAN_REVERSION_BALANCED',
                        'action': 'BUY',
                        'strength': 0.5,
                        'confluence_score': confluence['total'],
                        'reason': f'Retour VWAP équilibré: {vwap:.2f}'
                    })
            else:
                confluence = self._calculate_confluence_score(date, current_price, 'SELL')
                if self._apply_balanced_filters(date, current_price, 'SELL', confluence['total']):
    ...
    def run_balanced_backtest(self):
        """Exécuter backtest équilibré"""
        print("⚔️ Démarrage Battle Navale Équilibré Win Rate...")
        ...
        print(f"   📊 Paramètres équilibrés appliqués")
        print(f"   🔍 Filtres équilibrés activés")
        ...
        for i, (date, row) in enumerate(self.historical_data.iterrows()):
            ...
            # Générer signaux équilibrés
            signals = self._generate_balanced_signals(date, current_price)
    ...
    def _calculate_results(self, trades, equity_curve, initial_capital):
        """Calculer résultats"""
        print("📊 Calcul résultats Battle Navale Équilibré Win Rate...")
    ...
    def save_results(self):
        ...
        filename = f"data/test_results/battle_navale_equilibre_win_rate_{timestamp}.json"
    ...
    def run_complete_backtest(self):
        ...
        print("⚔️ BATTLE NAVALE ÉQUILIBRÉ WIN RATE - MIA_IA SYSTEM")
        print("=" * 60)
        ...
        # 2. Exécuter Battle Navale équilibré
        if not self.run_balanced_backtest():
            return False
        ...
        # 4. Résultats finaux
        print("\n" + "=" * 60)
        print("📊 RÉSULTATS BATTLE NAVALE ÉQUILIBRÉ WIN RATE")
        print("=" * 60)
        ...
        if (trading.get('total_trades', 0) > 20 and 
            ...
            print("\n🎉 BATTLE NAVALE ÉQUILIBRÉ WIN RATE RÉUSSI !")
            self.test_results['success'] = True
        else:
            print("\n⚠️ Battle Navale équilibré Win Rate - optimisations nécessaires")
...
def main():
    ...
    backtest = BattleNavaleEquilibreWinRate()
    success = backtest.run_complete_backtest()
    
    if success:
        print("\n🎉 BATTLE NAVALE ÉQUILIBRÉ WIN RATE VALIDÉ !")
    else:
        print("\n⚠️ Battle Navale équilibré Win Rate nécessite des optimisations")
