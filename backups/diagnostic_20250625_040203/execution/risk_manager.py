...
class RiskManager:
    ...
    def _check_basic_conditions(self) -> RiskDecision:
        ...
        if self.metrics.daily_pnl >= self.params.daily_profit_target:
            if not self.params.data_collection_mode:
                ...
                logger.info(f"Target atteint (${self.metrics.daily_pnl:.0f}) mais on continue (mode data collection)")
    ...
    def _check_signal_quality(self, signal: TradingSignal) -> RiskDecision:
        ...
        if hasattr(signal, 'confidence') or hasattr(signal, 'probability'):
            ...
            if signal_prob < self.params.min_signal_probability:
                return RiskDecision(
                    action=RiskAction.REJECT,
                    reason=f"Probabilité insuffisante: {signal_prob:.2f} < {self.params.min_signal_probability}"
                )
        ...
        if hasattr(signal, 'base_quality'):
            if signal.base_quality < self.params.min_base_quality_for_trade:
                if not self.params.data_collection_mode:
                    ...
                    logger.info(f"Base quality faible ({signal.base_quality:.2f}) mais acceptée (mode data collection)")
        ...
        if hasattr(signal, 'confluence_score'):
            if signal.confluence_score < self.params.min_confluence_score:
                if not self.params.data_collection_mode:
                    ...
                    logger.info(f"Confluence faible ({signal.confluence_score:.2f}) mais acceptée (mode data collection)")
    ...
    def _update_metrics(self):
        ...
        self.metrics.current_drawdown = (drawdown_dollars / self.metrics.peak_equity) * 100 if self.metrics.peak_equity > 0 else 0
    ...
    def force_close_all_positions(self, reason: str = "Force close"):
        ...
        logger.warning(f"⚠️ Force close all positions: {reason}")
...
def test_risk_manager():
    ...
    logger.info("Decision: {decision.action.value}")
    logger.info("   Size: {decision.approved_size}")
    logger.info("   Stop: {decision.stop_loss_price}")
    logger.info("   TP: {decision.take_profit_price}")
    logger.info("   Risk: ${decision.max_loss_dollars:.0f}")
    logger.info("   Score: {decision.risk_score:.0f}")
