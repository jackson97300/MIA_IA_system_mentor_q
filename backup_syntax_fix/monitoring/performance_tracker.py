...
📈 MÉTRIQUES PERFORMANCE DÉTAILLÉES & ANALYTICS AVANCÉES
...
1. 📊 TRACKING TRADING - P&L, Win Rate, Risk Metrics temps réel
2. 📉 ANALYTICS FINANCIÈRES - Sharpe, Sortino, Drawdown, VAR
3. 🔍 ANALYSE PÉRIODES - Détection patterns, cycles performance
4. 📋 REPORTING AVANCÉ - Rapports quotidiens/hebdomadaires professionnels
5. 🚨 ALERTES PERFORMANCE - Seuils risque, dégradation détection
6. 📈 ATTRIBUTION ANALYSE - Performance par stratégie/symbole/période
...
class PerformanceTracker:
    ...
    def _convert_to_trade_metrics(self, trade_result: Dict) -> Optional[TradeMetrics]:
        ...
        try:
            ...
            # Risk reward ratio
            risk_amount = abs(trade_result.get('stop_loss', entry_price) - entry_price) * quantity * ES_TICK_VALUE
            if risk_amount > 0:
    ...
    def _update_equity_curve(self, trade_metrics: TradeMetrics):
        ...
        try:
            ...
            if self.equity_curve:
                current_equity = self.equity_curve[-1] + trade_metrics.net_pnl
            else:
    ...
    def calculate_max_drawdown(self) -> Tuple[float, Optional[DrawdownPeriod]]:
        ...
        try:
            ...
            drawdowns = (equity_values - running_max) / np.maximum(running_max, 1)  # Éviter division par 0
            ...
            drawdown_period = DrawdownPeriod(
                ...
                recovery_time_days=(timestamps[recovery_idx] - timestamps[peak_idx]).days if recovery_idx else None,
                trades_during_drawdown=recovery_idx - peak_idx if recovery_idx else len(equity_values) - peak_idx,
            ...
            for i, equity in enumerate(equity_values):
                if equity > running_max:
                    ...
                    if in_drawdown:
                        ...
                        drawdown_end = i
                        min_equity_idx = drawdown_start_idx + np.argmin(equity_values[drawdown_start_idx:i])
                        
                        dd_percent = abs((equity_values[min_equity_idx] - drawdown_peak_value) / drawdown_peak_value) * 100
                        ...
                        drawdown_period = DrawdownPeriod(
                            ...
                            recovery_time_days=(timestamps[i] - timestamps[drawdown_start_idx]).days,
    ...
    def _create_performance_snapshot(self, period: PerformancePeriod, metrics: Dict) -> PerformanceSnapshot:
    ...
    def _check_performance_alerts(self, snapshot: PerformanceSnapshot) -> List[PerformanceAlert]:
        ...
        try:
            ...
            # Sharpe ratio alert
            if snapshot.total_trades >= 10 and snapshot.sharpe_ratio < self.alert_thresholds['min_sharpe_ratio']:
                alerts.append(PerformanceAlert(
                    timestamp=datetime.now(timezone.utc),
            ...
            if abs(snapshot.current_streak) >= self.alert_thresholds['max_consecutive_losses'] and snapshot.current_streak < 0:
                alerts.append(PerformanceAlert(
                    timestamp=datetime.now(timezone.utc),
                    alert_level=AlertLevel.WARNING,
            ...
            # Win rate alert
            if snapshot.total_trades >= 20 and snapshot.win_rate < self.alert_thresholds['min_win_rate_percent']:
                alerts.append(PerformanceAlert(
                    timestamp=datetime.now(timezone.utc),
                    alert_level=AlertLevel.WARNING,
                    metric='win_rate',
            ...
            # Profit factor alert
            if snapshot.total_trades >= 10 and snapshot.profit_factor < self.alert_thresholds['min_profit_factor']:
                alerts.append(PerformanceAlert(
                    timestamp=datetime.now(timezone.utc),
                    alert_level=AlertLevel.WARNING,
    ...
    def generate_performance_report(self, period: PerformancePeriod = PerformancePeriod.DAILY) -> str:
        """
        ...
        try:
            ...
            report_lines.append(f"Série Actuelle:      {abs(snapshot.current_streak)} {streak_type}")
...
def test_performance_tracker():
    ...
    logger.info("📈 TEST PERFORMANCE TRACKER")
    print("=" * 40)
    ...
    logger.info("🎯 Performance tracker test COMPLETED")
    return True
