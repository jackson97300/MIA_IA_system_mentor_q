...
class AutomationManager:
    ...
    def start_live_trading(self,
        ...
        logger.warning("⚠️  ATTENTION: TRADING AVEC CAPITAL RÉEL ⚠️")
        ...
        try:
            ...
            if not self._validate_live_trading_safety():
                logger.error("🚨 VALIDATION SÉCURITÉ ÉCHEC - ARRÊT")
                self.status = SystemStatus.ERROR
                return False
            ...
            logger.info("🚀 Démarrage trader en mode LIVE...")
            ...
            logger.info("✅ Live trading démarré avec CAPITAL RÉEL")
    ...
    def _validate_paper_trading_config(self) -> bool:
        ...
        logger.info("✅ Configuration paper trading validée")
        return True

    def _validate_live_trading_safety(self) -> bool:
        """Validation sécurité critique pour live trading"""
        logger.info("🔍 Validation sécurité live trading...")
        ...
        if hasattr(self.auto_config, 'max_daily_loss') and self.auto_config.max_daily_loss > 0:
            ...
            logger.error("❌ Limites de risque non configurées")
            return False
        ...
        logger.info(f"✅ Validation sécurité: {len(safety_checks)} checks OK")
    ...
    def _confirm_live_trading(self) -> bool:
        ...
        logger.warning("⚠️  CONFIRMATION REQUISE POUR LIVE TRADING ⚠️")

        # En production, demander confirmation interactive
        # Pour ce script, on simule une confirmation

        confirmation_text = """
        🚨 ATTENTION: TRADING AVEC CAPITAL RÉEL 🚨
    ...
    def _setup_live_trading_limits(self, capital_limit, max_position_size):
        ...
        logger.warning(f"🔒 Limites STRICTES live trading: {limits}")
    ...
    def _check_system_resources(self):
        ...
        try:
            ...
            if cpu_percent > 90:
                logger.warning(f"⚠️ CPU élevé: {cpu_percent}%")
                self.health = AutomationHealth.WARNING

            if memory.percent > 90:
                logger.warning(f"⚠️ Mémoire élevée: {memory.percent}%")
                self.health = AutomationHealth.WARNING
    ...
    def _live_trading_health_check(self):
        ...
        if checks_passed == total_checks:
            ...
            logger.error(f"🚨 SANTÉ CRITIQUE: {checks_passed}/{total_checks} checks OK")
    ...
    def _close_all_positions_emergency(self):
        ...
        logger.warning("🚨 Fermeture d'urgence positions...")
...
def main():
    ...
    try:
        ...
        if args.mode == "paper":
            ...
            if success:
                logger.info("🚨 ARRÊT D'URGENCE EFFECTUÉ")
            else:
...
def test_automation_starter():
    ...
    logger.info("Paper trading: {'✅' if success else '❌'}")
    ...
    if success:
        ...
        logger.info("Stop: {'✅' if stop_success else '❌'}")

    logger.info("\nTest 2: Data Collection")
    success = start_data_collection(collection_hours=1, target_samples=100)
    logger.info("Data collection: {'✅' if success else '❌'}")
    ...
    logger.info("Emergency stop: {'✅' if emergency_success else '❌'}")
