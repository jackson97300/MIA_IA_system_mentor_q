...
class LiveMonitor:
    ...
    def _render_dashboard(self) -> str:
        ...
        try:
            ...
            for name, health in self.components.items():
                ...
                components_html += f'<p class="{status_class}">• {
                    name.title()}: {
                    health.status.value.title()}</p>'
            ...
            if not recent_alerts:
                ...
                for alert in recent_alerts:
                    ...
                    recent_alerts_html += f'<div class="{alert_class}">[{time_str}] {
                        alert.component}: {
                        alert.message}</div>'
            ...
            html = html.replace('{{position_size}}',
                                str(self.trading_metrics.current_position_size))
    ...
    async def stop_monitoring(self):
        ...
        try:
            ...
            status_file = Path(
                "logs") / f"monitor_final_status_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
