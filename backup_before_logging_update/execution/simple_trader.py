...
class SimpleBattleNavaleTrader:
    ...
    def log_risk_diagnostics(self):
        ...
        logger.info(
            f"Sierra Address:Port:       {
                self.sierra_config.sierra_chart.server_address}:{
                self.sierra_config.sierra_chart.server_port}")
    ...
    def log_sierra_ibkr_diagnostics(self):
        ...
        logger.info(
            f"Historical Data:           {
                '✅' if config.ibkr.enable_historical_data else '❌'}")
        ...
        logger.info(
            f"Trading Enabled:           {
                '✅' if config.sierra_chart.trading_enabled else '❌'}")
        logger.info(
            f"Auto Connect:              {
                '✅' if config.sierra_chart.auto_connect else '❌'}")
        ...
        logger.info(
            f"Order Validation:          {
                '✅' if config.sierra_chart.enable_order_validation else '❌'}")
        logger.info(
            f"Position Tracking:         {
                '✅' if config.sierra_chart.enable_position_tracking else '❌'}")
        ...
        logger.info(
            f"Auto Rollover:             {
                '✅' if config.contracts.enable_auto_rollover else '❌'}")
        ...
        logger.info(
            f"Order Validation:          {
                '✅' if config.security.enable_order_validation else '❌'}")
        ...
        logger.info(
            f"Kill Switch Enabled:       {
                '✅' if config.security.enable_kill_switch else '❌'}")
        logger.info(f"Kill Switch Threshold:     ${config.security.kill_switch_loss_threshold:.0f}")
        logger.info(
            f"Fat Finger Protection:     {
                '✅' if config.security.enable_fat_finger_protection else '❌'}")
        logger.info(
            f"Real-time Monitoring:      {'✅' if config.security.enable_real_time_monitoring else '❌'}")

        # Synchronisation Configuration
        logger.info(f"\n🔄 SYNCHRONISATION CONFIGURATION")
        logger.info(
            f"Position Sync:             {
                '✅' if config.synchronization.enable_position_sync else '❌'}")
        logger.info(
            f"Order Sync:                {
                '✅' if config.synchronization.enable_order_sync else '❌'}")
        logger.info(
            f"Reconciliation:            {
                '✅' if config.synchronization.enable_reconciliation else '❌'}")
        logger.info(f"Sync Interval:             {config.synchronization.sync_interval_seconds}s")
        logger.info(
            f"Sierra Chart Priority:     {
                '✅' if config.synchronization.sierra_chart_priority else '❌'}")
        logger.info(
            f"IBKR Data Priority:        {
                '✅' if config.synchronization.ibkr_data_priority else '❌'}")
        logger.info(
            f"Failover Enabled:          {
                '✅' if config.synchronization.enable_failover else '❌'}")
    ...
    async def _execute_trade(self, signal: Any, decision: Any,
        ...
        try:
            ...
            if order_result.get('status') == 'FILLED':
                ...
                logger.info(
                    f"✅ Trade exécuté: {position.side} {position.size} @ {position.entry_price}")
    ...
    async def _close_position(self, position_id: str, exit_reason: str,
        ...
        try:
            ...
            logger.info(
                f"✅ Position fermée: {exit_reason} - P&L: ${final_pnl:.2f} ({pnl_ticks:.1f} ticks)")
