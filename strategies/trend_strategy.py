...
class TrendStrategy:
        ...
        if signal.confluence_score < required_confluence:
            logger.warning(
                f"Confluence insuffisante: {
                    signal.confluence_score:.2f} < {
                    required_confluence:.2f}")
            return False
    ...
    def _update_stats(self, signal: TrendSignalData):
        ...
        self.stats['trend_strength_avg'] = (
            (prev_trend_avg * (count - 1)) + signal.trend_structure.strength) / count
        self.stats['avg_confluence_score'] = (
            (prev_conf_avg * (count - 1)) + signal.confluence_score) / count
...
def test_trend_strategy():
    ...
    logger.info("\n[TARGET] TREND STRATEGY TEST COMPLETED")
    return True
