...
Exports corrigés avec gestion des imports circulaires
Version: Production Ready

Ce module gère l'export de tous les types core avec protection
...
# === IMPORTS NIVEAU 1 (dépendent de base_types) ===

# Structure Data - Import conditionnel pour éviter circularité
try:
    ...
    # Définir des placeholders pour éviter les erreurs
    StructureData = None
    Level = None
    VWAPBands = None
    MarketProfile = None
...
# Battle Navale - Import des classes réelles
try:
...
def check_core_availability() -> dict:
    """Vérifie la disponibilité des composants core"""
    return {
...
if __name__ == "__main__":
    logger.debug("DIAGNOSTIC CORE PACKAGE")
    print("=" * 50)
    ...
    logger.info("Status: {status['status'].upper()}")
    ...
    if not STRUCTURE_DATA_AVAILABLE:
        logger.info("\n⚠️  StructureData non disponible - Vérifier imports circulaires")
    
    if status['status'] == 'degraded':
        logger.info("\n⚠️  Certains composants manquent - Vérifier les imports")
    else:
        logger.info("\n✅ Core package opérationnel")