...
// NBCV Mapping corrigé : SG6(Ask Vol), SG7(Bid Vol), SG1(Ask-Bid Diff), SG12(Trades), SG10(Cumulative Sum)
...
SCSFExport scsf_MIA_Chart_Dumper_Patched_VIX_Daily(SCStudyInterfaceRef sc)
{
  if (sc.SetDefaults)
  {
    ...
    // --- NBCV par index SG (sans noms) ---
    sc.Input[19].Name = "Collect NBCV (SG index only)";
    sc.Input[19].SetYesNo(true);

    sc.Input[20].Name = "NBCV Study ID";
    sc.Input[20].SetInt(33); // ID exact de votre NBCV

    sc.Input[21].Name = "SG Ask Total";  sc.Input[21].SetInt(6);   // SG6 : Ask Volume Total
    sc.Input[22].Name = "SG Bid Total";  sc.Input[22].SetInt(7);   // SG7 : Bid Volume Total
    sc.Input[23].Name = "SG Delta";      sc.Input[23].SetInt(1);   // SG1 : Ask - Bid
    sc.Input[24].Name = "SG Trades";     sc.Input[24].SetInt(12);  // SG12 : Number of Trades
    sc.Input[25].Name = "SG CumDelta";   sc.Input[25].SetInt(10);  // SG10 : Cum (Ask-Bid) Day
    sc.Input[26].Name = "NBCV On New Bar Only"; sc.Input[26].SetInt(1);
  ...
  // ===== NBCV INDEX-ONLY (SG1,6,7,10,12) =====
  static int last_nbcv_bar = -1;
  if (sc.Input[19].GetYesNo() && sc.ArraySize > 0) {
    const int i = sc.ArraySize - 1;
    if (sc.Input[26].GetInt() != 0) {
      if (i == last_nbcv_bar) goto nbcv_idx_done;
      last_nbcv_bar = i;
    }

    const int nbcv_id = sc.Input[20].GetInt();
    if (nbcv_id <= 0) goto nbcv_idx_done;

    const int sgAsk = sc.Input[21].GetInt();
    const int sgBid = sc.Input[22].GetInt();
    const int sgDel = sc.Input[23].GetInt();
    const int sgTrd = sc.Input[24].GetInt();
    ...
    if (!(has(aAsk) && has(aBid))) goto nbcv_idx_done;

    const double t   = sc.BaseDateTimeIn[i].GetAsDouble();
    const double ask = aAsk[i];
    const double bid = aBid[i];
    const double delta = has(aDel) ? aDel[i] : (ask - bid);  // fallback si SG1 vide
    const double trades = has(aTrd) ? aTrd[i] : 0.0;
    const double cum   = has(aCum) ? aCum[i] : 0.0;
    const double tot   = ask + bid;

    // Ligne unique, propre
    SCString j;
    j.Format(R"({"t":%.6f,"sym":"%s","type":"nbcv","i":%d,"ask":%.0f,"bid":%.0f,"delta":%.0f,"trades":%.0f,"cumdelta":%.0f,"total":%.0f})",
             t, sc.Symbol.GetChars(), i, ask, bid, delta, trades, cum, tot);
    WritePerChartDaily(sc.ChartNumber, j);
  ...
  if ((sc.Input[27].GetYesNo() || sc.Input[28].GetYesNo()) && sc.ArraySize > 0)
  {
    ...
    if (newbar_only && i == s_last_ts_bar) {
      ...
      if (sc.Input[28].GetYesNo()) {
        const double t = sc.BaseDateTimeIn[i].GetAsDouble();
        const double bid = sc.Bid;
        const double ask = sc.Ask;
