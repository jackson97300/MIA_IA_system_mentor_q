...
class PerformanceAnalyzer:
    ...
    def _load_trades_data(self, start_date: Optional[datetime], end_date: Optional[datetime]) -> pd.DataFrame:
    ...
    def _load_snapshots_data(self, start_date: Optional[datetime], end_date: Optional[datetime]) -> pd.DataFrame:
    ...
    def _analyze_daily_trades(self) -> Dict[str, Any]:
        ...
        signal_performance = self.trades_data.groupby('signal_type')['pnl'].agg(['sum', 'mean', 'count']).to_dict()
    ...
    def _generate_daily_alerts(self) -> List[PerformanceAlert]:
        ...
        if self.current_metrics.max_consecutive_losses >= self.alert_thresholds["max_consecutive_losses"]:
            alerts.append(PerformanceAlert(
                ...
                message=f"Trop de pertes consécutives: {self.current_metrics.max_consecutive_losses}"
            ))
    ...
    def _generate_attribution_recommendations(self, contribution_analysis: Dict[str, Any]) -> List[str]:
    ...
    def _create_performance_heatmap(self, filepath: Path, output_format: str):
        ...
        heatmap_data = self.trades_data.groupby(['weekday', 'hour'])['pnl'].sum().unstack(fill_value=0)
...
def analyze_trade_patterns(pattern_types: List[PatternType] = None, lookback_days: int = 30) -> Dict[str, Any]:
    """
...
def export_performance_charts(chart_types: List[str] = None, output_format: str = "html") -> List[str]:
    """
...
def main():
    ...
    parser.add_argument("--format", choices=["html", "png", "pdf"], default="html", help="Format export")
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(f"logs/performance/analysis_{datetime.now().strftime('%Y%m%d')}.log"),
    ...
    try:
        ...
        if args.command == "daily":
            ...
            logger.info("📊 CHARTS EXPORTÉS: {len(files)} fichiers")
            for file in files[:5]:  # Afficher les 5 premiers
                logger.info("  • {Path(file).name}")
        
        elif args.command == "full":
            logger.info("🔄 ANALYSE COMPLÈTE EN COURS...")
...
def test_performance_analyzer():
    ...
    logger.info("Chargement données: {'✅' if success else '❌'}")
    
    if success:
        logger.info("Test 2: Rapport quotidien")
        daily_report = generate_daily_report()
        logger.info("Rapport quotidien: {'✅' if 'summary' in daily_report else '❌'}")
        
        logger.info("Test 3: Analyse patterns")
        patterns = analyze_trade_patterns(lookback_days=7)
        logger.info("Analyse patterns: {'✅' if 'patterns_analysis' in patterns else '❌'}")
        
        logger.info("Test 4: Attribution")
        attribution = calculate_strategy_attribution(7)
        logger.info("Attribution: {'✅' if 'strategy_performance' in attribution else '❌'}")
        
        logger.info("Test 5: Export charts")
        charts = export_performance_charts(["equity_curve", "drawdown"])
        logger.info("Export charts: {'✅' if len(charts) > 0 else '❌'}")
