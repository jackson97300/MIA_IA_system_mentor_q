...
try:
    ...
    logger.info("🔄 Mode déploiement standalone activé")
...
class LiveDeployer:
    ...
    def _request_manual_approval(self) -> bool:
        ...
        try:
            ...
            print("\n" + "="*60)
            logger.info("🚨 MANUAL APPROVAL REQUIRED FOR LIVE TRADING")
            print("="*60)
        ...
        while self.is_running and (time.time() - monitoring_start) < max_monitoring_time:
            try:
                ...
                if int(time.time()) % 300 == 0:  # Tous les 5 minutes
                    logger.info(
                        f"📊 Monitoring: Health {current_health:.1f}%, Uptime {int(time.time() - monitoring_start)}s")
...
Exemples d'utilisation:
  ...
  python scripts/deploy_live.py --health-check
    ...
    try:
        logger.info("🚀 MIA_IA_SYSTEM LIVE DEPLOYMENT SYSTEM")
        print("=" * 60)
        ...
        logger.info("🚀 Starting deployment: {args.mode}")
        ...
        if report.overall_status == DeploymentStatus.SUCCESS:
            ...
            if SYSTEM_INTEGRATION and not args.live:
                logger.info("🌐 Health Dashboard: http://127.0.0.1:8080")
