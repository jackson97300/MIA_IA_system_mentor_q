...
class AlertLevel(Enum):
    """Niveaux d'alerte pour le système de monitoring"""
    DEBUG = "debug"
    INFO = "info"
    WARNING = "warning"
    ERROR = "error"
...
class AutomationMonitoringConfig:
    ...
    default_alert_level: AlertLevel = AlertLevel.INFO
        ...
        if 'monitoring' in data:
            for key, value in data['monitoring'].items():
                if hasattr(config.monitoring, key):
                    if key == 'default_alert_level' and isinstance(value, str):
                        setattr(config.monitoring, key, AlertLevel(value))
                    else:
