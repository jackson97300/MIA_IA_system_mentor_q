...
class TrainingScriptManager:
    ...
    def train_from_snapshots(self,
        ...
        try:
            print("=" * 60)
            logger.info("[LAUNCH] DÉBUT TRAINING MODÈLE BATTLE NAVALE")
            print("=" * 60)
            ...
            logger.info("[DATE] Période données: {start_dt.date()} → {end_dt.date()}")
            logger.info("[CONFIG] Mode training: {training_mode}")
            logger.info("[TARGET] Type modèle: {self.trainer.config.model_type.value}")
            print()
            ...
            # Résultats
            print("=" * 60)
            logger.info("[STATS] RÉSULTATS TRAINING")
            print("=" * 60)
            ...
            if session.model_performance:
                ...
                if perf['accuracy'] >= target_acc:
                    logger.info("[TARGET] Accuracy atteint la cible ({target_acc:.2f})")
                else:
                    logger.info("[UP] Accuracy sous la cible ({target_acc:.2f})")
            ...
            if session.validation_results:
                ...
                logger.info(
                    "  Production Ready: {'[OK]' if val_results.get('production_readiness', False) else '[ERROR]'}")
            ...
            if success:
                ...
                if session.model_path:
                    logger.info("[FOLDER] Modèle sauvegardé: {session.model_path}")
            else:
    ...
    def setup_continuous_learning(self,
        ...
        """
        try:
            logger.info("[SYNC] CONFIGURATION APPRENTISSAGE CONTINU")
            print("=" * 50)
    ...
    def deploy_model(self, version_id: str, stage: str = "production") -> bool:
        ...
        """
        try:
            logger.info("[LAUNCH] DÉPLOIEMENT MODÈLE → {stage.upper()}")
            print("=" * 50)
    ...
    def show_training_status(self, detailed: bool = False) -> Dict[str, Any]:
        ...
        """
        try:
            logger.info("[STATS] STATUT SYSTÈME TRAINING ML")
            print("=" * 50)
            ...
            logger.info(
                "[CONFIG] Trainer actif: {'[OK]' if sys_status.get('trainer_active', False) else '[ERROR]'}")
            logger.info(
                "[SYNC] Apprentissage continu: {'[OK]' if sys_status.get('continuous_learning', False) else '[ERROR]'}")
            logger.info("[UP] Modèles total: {sys_status.get('models_total', 0)}")
            logger.info("[LAUNCH] Modèles production: {sys_status.get('models_in_production', 0)}")
            ...
            if active:
                ...
                if active['deployment_date']:
                    dep_date = datetime.fromisoformat(
                        active['deployment_date']).strftime('%Y-%m-%d %H:%M')
            ...
            logger.info("  Auto deploy: {'[OK]' if config.get('auto_deploy', False) else '[ERROR]'}")
    ...
    def retrain_if_needed(self) -> bool:
        ...
        try:
            ...
            if session:
                logger.info("[SYNC] Re-training automatique lancé:")
                logger.info("  Session: {session.session_id}")
                logger.info("  Status: {session.status.value}")

                # Attendre fin training
                while session.status in [TrainingStatus.PREPARING_DATA,
                                         TrainingStatus.TRAINING, TrainingStatus.VALIDATING]:
...
Exemples:
  ...
  python scripts/train_models.py --deploy v_training_20231215_143022
    ...
    try:
        ...
        logger.info("\n[STOP]  Arrêt demandé par utilisateur")
        sys.exit(1)
    except Exception as e:
