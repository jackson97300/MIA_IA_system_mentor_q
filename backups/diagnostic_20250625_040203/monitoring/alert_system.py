...
try:
    ...
    logger.warning("Email functionality disabled: {e}")
...
def test_alert_system():
    ...
    logger.info("System health: CPU {health.cpu_percent:.1f}%, RAM {health.memory_percent:.1f}%")
    
    # Test alerte factice
    fake_performance = {'max_drawdown_percent': 15.0}
    alert_system.monitor_risk_thresholds(fake_performance)
    logger.info("Test alerte drawdown")
    ...
    logger.info("Alertes générées: {stats['session_stats']['alerts_generated']}")
    logger.info("Email disponible: {stats['email_available']}")
...
if __name__ == "__main__":
    logger.info("Email functionality: {'✅ Available' if EMAIL_AVAILABLE else '❌ Disabled (MIMEText import failed)'}")
    test_alert_system()