...
class ImportDiagnostic:
    ...
    def scan_project(self):
        ...
        for py_file in self.project_root.rglob("*.py"):
            ...
            try:
                ...
                logger.info("   ❌ Error analyzing {py_file}: {e}")
    ...
    def _analyze_file(self, file_path: Path) -> ModuleInfo:
        ...
        try:
            ...
            logger.info("   ⚠️ Syntax error in {file_path}: {e}")
    ...
    def print_report(self):
        ...
        print("\n" + "="*60)
        logger.info("📊 IMPORT DIAGNOSTIC REPORT")
        print("="*60)
        ...
        for module_path, module_info in sorted(self.modules.items()):
            ...
            logger.info("   • {module_path}: {classes_count}C, {functions_count}F, {variables_count}V")
        ...
        for issue_type, issues in by_type.items():
            ...
            for issue in issues:
                logger.info("   📄 {issue.file_path.name}:{issue.line_number}")
                logger.info("      ❌ {issue.import_statement}")
                logger.info("      🎯 '{issue.name}' not found in '{issue.module}'")
                if issue.suggestion:
    ...
    def suggest_corrections(self):
        ...
        for file_path, issues in by_file.items():
            ...
            for issue in issues:
                logger.info("   Line {issue.line_number}: {issue.import_statement}")
                ...
                if issue.suggestion:
                    if issue.issue_type == 'incorrect':
                        ...
                        logger.info("   ✅ Suggested: {corrected}")
                    else:
                    ...
                    if available.get('all'):
                        logger.info("   🔍 Available in '{issue.module}':")
            ...
            if not dry_run:
                ...
                for issue in sorted(issues, key=lambda x: x.line_number, reverse=True):
                    ...
                    if line_idx < len(lines):
                        ...
                        fixes_applied += 1
                        logger.info("Fixed {file_path.name}:{issue.line_number}")
                        logger.info("   Old: {old_line.strip()}")
                        logger.info("   New: {new_line.strip()}")
                ...
                for issue in issues:
                    ...
                    new_line = old_line.replace(issue.name, issue.suggestion)
                    logger.info("📄 {file_path.name}:{issue.line_number}")
                    logger.info("   Would change: {old_line}")
                    logger.info("   To:          {new_line}")
                    fixes_applied += 1
            ...
            if match:
                ...
                if module_name in self.modules:
                    ...
                    if available['classes']:
                        logger.info("   📦 Classes: {', '.join(sorted(available['classes']))}")
                    if available['functions']:
                        logger.info("   🔧 Functions: {', '.join(sorted(available['functions']))}")
                    if available['variables']:
                        logger.info("   📊 Variables: {', '.join(sorted(available['variables']))}")
                    ...
                    logger.error("Module '{module_name}' not found in project")
...
def main():
    ...
    logger.info("🚀 MIA_IA_SYSTEM - IMPORT DIAGNOSTIC")
    print("="*60)
    logger.info("📂 Project root: {Path(project_root).absolute()}")
    ...
    print("\n" + "="*60)
    logger.debug("SPECIFIC MODULE ANALYSIS")
    print("="*60)
    ...
    if 'core.battle_navale' in diagnostic.modules:
        ...
        logger.info("📦 core.battle_navale available:")
        for category, names in available.items():
            if names and category != 'all':
                logger.info("   {category.upper()}: {', '.join(sorted(names))}")
    else:
        logger.error("core.battle_navale module not found!")
    ...
    logger.info("1. Check if 'BattleNavaleDetector' class exists in core/battle_navale.py")
    logger.info("2. Verify __all__ exports in each module")
    logger.info("3. Run this script with auto-fix to correct obvious issues")
    logger.info("4. Update __init__.py files to match actual available classes/functions")
    
    logger.info("\n🎯 Diagnostic complete! Found {len(diagnostic.issues)} issues.")
    
    # Quick fix suggestions for the specific error
    print("\n" + "="*60)
    logger.info("🚑 QUICK FIX FOR CURRENT ERROR")
    print("="*60)
    
    logger.info("Current error: cannot import name 'BattleNavaleDetector' from 'core.battle_navale'")
    logger.info("\nPossible solutions:")
    logger.info("1. Check if the class name is correct in battle_navale.py")
    logger.info("2. Add 'BattleNavaleDetector' to __all__ in battle_navale.py")
    logger.info("3. Update core/__init__.py to import the correct name")
    logger.info("4. Verify the class is actually defined and not commented out")
