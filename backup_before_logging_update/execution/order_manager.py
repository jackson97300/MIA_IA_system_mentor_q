...
class OrderManager:
    ...
    def __init__(self, mode: str = "simulation", sierra_config: Optional[SierraIBKRConfig] = None):
        ...
        logger.info(
            f"  - Order Provider: {
                self.order_provider.value if hasattr(
                    self.order_provider,
                    'value') else self.order_provider}")
        ...
        try:
            ...
            if self.sierra_config:
                ...
                if order.size > security_config.max_single_position:
                    return {
                        'valid': False, 'error': f'Position > limite sécurité ({security_config.max_single_position})'}
    ...
    def log_diagnostics(self):
        ...
        logger.info(
            f"Kill Switch:               {
                '🚨 ACTIVÉ' if self.kill_switch_triggered else '✅ Normal'}")
...
def create_order_manager(mode: str = "simulation",
