...
# -*- coding: utf-8 -*-
"""
Battle Navale Win Rate Final Success - MIA_IA System
Version finale optimisée pour Win Rate 66.7%+ avec paramètres ajustés
"""
...
class BattleNavaleWinRateFinalSuccess:
    def __init__(self):
        self.test_results = {
        ...
        # Paramètres OPTIMISÉS POUR WIN RATE ÉLEVÉ
        self.params = {
            'position_size': 0.06,     # 6% - Plus conservateur
            'take_profit': 0.015,      # 1.5% - Plus rapide pour Win Rate
            'stop_loss': 0.012,        # 1.2% - Plus large pour éviter les stops
            'max_positions': 5         # Moins de positions simultanées
        }
        
        # Filtres OPTIMISÉS POUR WIN RATE
        self.filters = {
            'min_confluence': 0.50,    # Score confluence plus élevé pour qualité
    ...
    def load_data(self):
        ...
        print("📂 Chargement données pour Battle Navale Win Rate Final Success...")
    ...
    def _calculate_technical_indicators(self):
        """Calculer indicateurs techniques optimisés pour Win Rate"""
    ...
    def _calculate_confluence_score(self, date, current_price, signal_type):
        """Calculer score de confluence optimisé pour Win Rate"""
        confluence_factors = {
            'technical': 0,
            'volume': 0,
            'options': 0,
        ...
        # RSI avec seuils optimisés pour Win Rate
        rsi = row['RSI']
        ...
        # 7. FACTEURS SUPPORT/RESISTANCE (0% - désactivé pour Win Rate)
    ...
    def _apply_win_rate_filters(self, date, current_price, signal_type, confluence_score):
        """Appliquer filtres optimisés pour Win Rate élevé"""
        row = self.historical_data.loc[date]
    ...
    def _generate_win_rate_signals(self, date, current_price):
        """Générer signaux optimisés pour Win Rate élevé"""
        signals = []
        
        row = self.historical_data.loc[date]
        
        # 1. SIGNAL RSI (Seuils optimisés pour Win Rate)
        rsi = row['RSI']
        if rsi < 35:  # Survente optimisée
            confluence = self._calculate_confluence_score(date, current_price, 'BUY')
            if self._apply_win_rate_filters(date, current_price, 'BUY', confluence['total']):
                signals.append({
                    'type': 'RSI_OVERSOLD_WINRATE',
                    'action': 'BUY',
                    'strength': 0.8,
                    'confluence_score': confluence['total'],
                    'reason': f'RSI survente optimisé: {rsi:.1f}'
                })
        elif rsi > 65:  # Surachat optimisé
            confluence = self._calculate_confluence_score(date, current_price, 'SELL')
            if self._apply_win_rate_filters(date, current_price, 'SELL', confluence['total']):
                signals.append({
                    'type': 'RSI_OVERBOUGHT_WINRATE',
                    'action': 'SELL',
                    'strength': 0.8,
                    'confluence_score': confluence['total'],
                    'reason': f'RSI surachat optimisé: {rsi:.1f}'
                })
        ...
        if macd > macd_signal and macd > 0:  # Signal haussier confirmé
            ...
            if self._apply_win_rate_filters(date, current_price, 'BUY', confluence['total']):
                signals.append({
                    'type': 'MACD_BULLISH_WINRATE',
                    'action': 'BUY',
                    'strength': 0.7,
                    'confluence_score': confluence['total'],
                    'reason': f'MACD haussier optimisé: {macd:.4f}'
                })
        elif macd < macd_signal and macd < 0:  # Signal baissier confirmé
            confluence = self._calculate_confluence_score(date, current_price, 'SELL')
            if self._apply_win_rate_filters(date, current_price, 'SELL', confluence['total']):
                signals.append({
                    'type': 'MACD_BEARISH_WINRATE',
                    'action': 'SELL',
                    'strength': 0.7,
                    'confluence_score': confluence['total'],
                    'reason': f'MACD baissier optimisé: {macd:.4f}'
                })
        ...
        if current_price < bb_lower * 1.005:  # Proximité support
            ...
            if self._apply_win_rate_filters(date, current_price, 'BUY', confluence['total']):
                signals.append({
                    'type': 'BB_SUPPORT_WINRATE',
                    'action': 'BUY',
                    'strength': 0.9,
                    'confluence_score': confluence['total'],
                    'reason': f'Proximité support BB optimisé: {bb_lower:.2f}'
                })
        elif current_price > bb_upper * 0.995:  # Proximité résistance
            confluence = self._calculate_confluence_score(date, current_price, 'SELL')
            if self._apply_win_rate_filters(date, current_price, 'SELL', confluence['total']):
                signals.append({
                    'type': 'BB_RESISTANCE_WINRATE',
                    'action': 'SELL',
                    'strength': 0.9,
                    'confluence_score': confluence['total'],
                    'reason': f'Proximité résistance BB optimisé: {bb_upper:.2f}'
                })
        ...
        if date in self.options_data:
            ...
            if current_price < gex1 * 1.005:  # Proximité support GEX
                ...
                if self._apply_win_rate_filters(date, current_price, 'BUY', confluence['total']):
                    signals.append({
                        'type': 'GEX_SUPPORT_WINRATE',
                        'action': 'BUY',
                        'strength': 1.0,
                        'confluence_score': confluence['total'],
                        'reason': f'Proximité support GEX optimisé: {gex1:.2f}'
                    })
            elif current_price > gex2 * 0.995:  # Proximité résistance GEX
                confluence = self._calculate_confluence_score(date, current_price, 'SELL')
                if self._apply_win_rate_filters(date, current_price, 'SELL', confluence['total']):
                    signals.append({
                        'type': 'GEX_RESISTANCE_WINRATE',
                        'action': 'SELL',
                        'strength': 1.0,
                        'confluence_score': confluence['total'],
                        'reason': f'Proximité résistance GEX optimisé: {gex2:.2f}'
                    })
        
        # 5. SIGNAL VWAP (Optimisé pour Win Rate)
        vwap = row['VWAP']
        ...
        if vwap_distance > 0.008:  # Écart optimisé
            if current_price < vwap:
                ...
                if self._apply_win_rate_filters(date, current_price, 'BUY', confluence['total']):
                    signals.append({
                        'type': 'VWAP_MEAN_REVERSION_WINRATE',
                        'action': 'BUY',
                        'strength': 0.6,
                        'confluence_score': confluence['total'],
                        'reason': f'Retour VWAP optimisé: {vwap:.2f}'
                    })
            else:
                confluence = self._calculate_confluence_score(date, current_price, 'SELL')
                if self._apply_win_rate_filters(date, current_price, 'SELL', confluence['total']):
    ...
    def run_win_rate_backtest(self):
        """Exécuter backtest optimisé pour Win Rate"""
        print("⚔️ Démarrage Battle Navale Win Rate Final Success...")
        ...
        print(f"   📊 Paramètres optimisés pour Win Rate appliqués")
        print(f"   🔍 Filtres optimisés pour Win Rate activés")
        
        # Parcourir chaque jour
        for i, (date, row) in enumerate(self.historical_data.iterrows()):
            current_price = row['Close']
            
            # Générer signaux optimisés pour Win Rate
            signals = self._generate_win_rate_signals(date, current_price)
    ...
    def _calculate_results(self, trades, equity_curve, initial_capital):
        """Calculer résultats"""
        print("📊 Calcul résultats Battle Navale Win Rate Final Success...")
    ...
    def save_results(self):
        ...
        filename = f"data/test_results/battle_navale_win_rate_final_success_{timestamp}.json"
    ...
    def run_complete_backtest(self):
        ...
        print("⚔️ BATTLE NAVALE WIN RATE FINAL SUCCESS - MIA_IA SYSTEM")
        print("=" * 60)
        ...
        # 2. Exécuter Battle Navale optimisé pour Win Rate
        if not self.run_win_rate_backtest():
            return False
        ...
        # 4. Résultats finaux
        print("\n" + "=" * 60)
        print("📊 RÉSULTATS BATTLE NAVALE WIN RATE FINAL SUCCESS")
        print("=" * 60)
        ...
        # Critères de succès optimisés pour Win Rate
        if (trading.get('total_trades', 0) > 25 and 
            trading.get('win_rate', 0) >= 65 and 
            trading.get('profit_factor', 0) >= 1.2):
            print("\n🎉 BATTLE NAVALE WIN RATE FINAL SUCCESS RÉUSSI !")
            self.test_results['success'] = True
        else:
            print("\n⚠️ Battle Navale Win Rate final success - optimisations nécessaires")
...
def main():
    ...
    backtest = BattleNavaleWinRateFinalSuccess()
    success = backtest.run_complete_backtest()
    
    if success:
        print("\n🎉 BATTLE NAVALE WIN RATE FINAL SUCCESS VALIDÉ !")
    else:
        print("\n⚠️ Battle Navale Win Rate final success nécessite des optimisations")
