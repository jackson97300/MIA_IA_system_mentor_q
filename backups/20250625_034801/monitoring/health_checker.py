...
class HealthReport:
    ...
    def calculate_summary(self):
        ...
        self.healthy_components = len([c for c in self.components.values() if c.status == HealthStatus.HEALTHY])
        self.warning_components = len([c for c in self.components.values() if c.status == HealthStatus.WARNING])
        self.critical_components = len([c for c in self.components.values() if c.status == HealthStatus.CRITICAL])
        self.offline_components = len([c for c in self.components.values() if c.status == HealthStatus.OFFLINE])
        
        response_times = [c.response_time_ms for c in self.components.values() if c.response_time_ms > 0]
...
class HealthChecker:
    ...
    def _perform_health_checks(self):
        ...
        for comp_name, future in futures.items():
            try:
                updated_component = future.result(timeout=self.config['health_check_timeout_seconds'])
    ...
    def _check_component_health(self, comp_name: str, component: ComponentHealth) -> Optional[ComponentHealth]:
    ...
    def _determine_recovery_action(self, comp_name: str, component: ComponentHealth) -> RecoveryAction:
    ...
    def _execute_recovery_action(self, comp_name: str, component: ComponentHealth, action: RecoveryAction) -> bool:
    ...
    def _generate_health_report(self) -> HealthReport:
        ...
        try:
            current_time = datetime.now(timezone.utc)
            latest_metrics = self.system_metrics_history[-1] if self.system_metrics_history else SystemMetrics(timestamp=current_time)
