...
# -*- coding: utf-8 -*-
"""
Battle Navale Ultra-Simple - MIA_IA System
Version ultra-simplifiée avec seuils permissifs
"""
...
class BattleNavaleUltraSimple:
    def __init__(self):
        ...
        # Paramètres ULTRA-SIMPLIFIÉS
        self.params = {
            'position_size': 0.10,     # 10% - Plus agressif
            'take_profit': 0.015,      # 1.5% - Plus rapide
            'stop_loss': 0.008,        # 0.8% - Plus serré
            'max_positions': 10        # Plus de positions
        }
    ...
    def load_data(self):
        ...
        print("📂 Chargement données pour Battle Navale Ultra-Simple...")
    ...
    def _calculate_technical_indicators(self):
        """Calculer indicateurs techniques ultra-simplifiés"""
    ...
    def _generate_ultra_simple_signals(self, date, current_price):
        """Générer signaux ultra-simplifiés avec seuils permissifs"""
        signals = []
        
        row = self.historical_data.loc[date]
        
        # 1. SIGNAL RSI (Seuils élargis)
        rsi = row['RSI']
        if rsi < 40:  # Survente élargie (au lieu de 30)
            signals.append({
                'type': 'RSI_OVERSOLD',
                'action': 'BUY',
                'strength': 0.6,
        ...
        elif rsi > 60:  # Surachat élargi (au lieu de 70)
            signals.append({
                'type': 'RSI_OVERBOUGHT',
                'action': 'SELL',
                'strength': 0.6,
        ...
        if macd > macd_signal:  # Signal haussier (supprimé condition macd > 0)
            signals.append({
                'type': 'MACD_BULLISH',
                'action': 'BUY',
                'strength': 0.5,
        ...
        elif macd < macd_signal:  # Signal baissier (supprimé condition macd < 0)
            signals.append({
                'type': 'MACD_BEARISH',
                'action': 'SELL',
                'strength': 0.5,
        ...
        # 3. SIGNAL BOLLINGER BANDS (Proximité élargie)
        bb_upper = row['BB_Upper']
        bb_lower = row['BB_Lower']
        bb_distance = (bb_upper - bb_lower) / bb_lower
        
        if current_price < bb_lower * 1.02:  # Proximité support (2% au lieu de contact)
            signals.append({
                'type': 'BB_SUPPORT',
                'action': 'BUY',
                'strength': 0.7,
                'reason': f'Proche support BB: {bb_lower:.2f}'
            })
        elif current_price > bb_upper * 0.98:  # Proximité résistance (2% au lieu de contact)
            signals.append({
                'type': 'BB_RESISTANCE',
                'action': 'SELL',
                'strength': 0.7,
                'reason': f'Proche résistance BB: {bb_upper:.2f}'
            })
        
        # 4. SIGNAL VWAP (Écart réduit)
        vwap = row['VWAP']
        vwap_distance = abs(current_price - vwap) / vwap
        
        if vwap_distance > 0.005:  # Écart réduit (0.5% au lieu de 1%)
        ...
        # 5. SIGNAL GEX (Proximité élargie)
        if date in self.options_data:
            ...
            if current_price < gex1 * 1.03:  # Proximité support GEX (3%)
                signals.append({
                    'type': 'GEX_SUPPORT',
                    'action': 'BUY',
                    'strength': 0.8,
                    'reason': f'Proche support GEX: {gex1:.2f}'
                })
            elif current_price > gex2 * 0.97:  # Proximité résistance GEX (3%)
                signals.append({
                    'type': 'GEX_RESISTANCE',
                    'action': 'SELL',
                    'strength': 0.8,
                    'reason': f'Proche résistance GEX: {gex2:.2f}'
                })
        
        # 6. SIGNAL ORDER FLOW (Seuil réduit)
        if date in self.orderflow_data:
            ...
            if flow_direction == 'BULLISH' and flow_intensity > 1.0:  # Seuil réduit (1.0 au lieu de 1.5)
                signals.append({
            ...
            elif flow_direction == 'BEARISH' and flow_intensity > 1.0:  # Seuil réduit
                signals.append({
        ...
        # 7. SIGNAL TREND (Nouveau - basé sur prix)
        if len(self.historical_data) > 5:
            recent_prices = self.historical_data['Close'].tail(5)
            price_trend = (recent_prices.iloc[-1] - recent_prices.iloc[0]) / recent_prices.iloc[0] * 100
            
            if price_trend > 0.5:  # Tendance haussière
                signals.append({
                    'type': 'TREND_BULLISH',
                    'action': 'BUY',
                    'strength': 0.3,
                    'reason': f'Tendance haussière: {price_trend:.2f}%'
                })
            elif price_trend < -0.5:  # Tendance baissière
                signals.append({
                    'type': 'TREND_BEARISH',
                    'action': 'SELL',
                    'strength': 0.3,
                    'reason': f'Tendance baissière: {price_trend:.2f}%'
                })
        
        # 8. SIGNAL VOLUME (Nouveau)
        volume_ratio = row['Volume'] / self.historical_data['Volume'].rolling(20).mean().loc[date]
        
        if volume_ratio > 1.1:  # Volume élevé
            if current_price > row['Open']:  # Prix en hausse
                signals.append({
                    'type': 'VOLUME_BULLISH',
                    'action': 'BUY',
                    'strength': 0.4,
                    'reason': f'Volume haussier: {volume_ratio:.2f}x'
                })
            else:  # Prix en baisse
                signals.append({
                    'type': 'VOLUME_BEARISH',
                    'action': 'SELL',
                    'strength': 0.4,
                    'reason': f'Volume baissier: {volume_ratio:.2f}x'
                })
        
        return signals
    
    def run_ultra_simple_backtest(self):
        """Exécuter backtest ultra-simplifié"""
        print("⚔️ Démarrage Battle Navale Ultra-Simple...")
        ...
        print(f"   📊 Paramètres ultra-simplifiés appliqués")
        
        # Parcourir chaque jour
        for i, (date, row) in enumerate(self.historical_data.iterrows()):
            current_price = row['Close']
            
            # Générer signaux ultra-simplifiés
            signals = self._generate_ultra_simple_signals(date, current_price)
    ...
    def _calculate_results(self, trades, equity_curve, initial_capital):
        """Calculer résultats"""
        print("📊 Calcul résultats Battle Navale Ultra-Simple...")
    ...
    def save_results(self):
        ...
        filename = f"data/test_results/battle_navale_ultra_simple_{timestamp}.json"
    ...
    def run_complete_backtest(self):
        ...
        print("⚔️ BATTLE NAVALE ULTRA-SIMPLE - MIA_IA SYSTEM")
        print("=" * 60)
        ...
        # 2. Exécuter Battle Navale ultra-simple
        if not self.run_ultra_simple_backtest():
            return False
        ...
        # 4. Résultats finaux
        print("\n" + "=" * 60)
        print("📊 RÉSULTATS BATTLE NAVALE ULTRA-SIMPLE")
        print("=" * 60)
        ...
        if (trading.get('total_trades', 0) > 10 and 
            trading.get('win_rate', 0) >= 35 and 
            trading.get('profit_factor', 0) >= 1.05):
            print("\n🎉 BATTLE NAVALE ULTRA-SIMPLE RÉUSSI !")
            self.test_results['success'] = True
        else:
            print("\n⚠️ Battle Navale ultra-simple - optimisations nécessaires")
...
def main():
    ...
    backtest = BattleNavaleUltraSimple()
    success = backtest.run_complete_backtest()
    
    if success:
        print("\n🎉 BATTLE NAVALE ULTRA-SIMPLE VALIDÉ !")
    else:
        print("\n⚠️ Battle Navale ultra-simple nécessite des optimisations")
