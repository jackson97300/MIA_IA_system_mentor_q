...
try:
    ...
    logging.warning(
        "discord.py n'est pas installé - Les notifications Discord ne seront pas disponibles\n"
...
class DiscordConfig:
    ...
    # Token bot (OBLIGATOIRE si Discord activé)
    bot_token: str
...
class DiscordNotifier:
    """
    Notificateur Discord pour MIA_IA_SYSTEM
    ...
    def __init__(self, config: Optional[DiscordConfig] = None):
        ...
        if not DISCORD_AVAILABLE:
            raise ImportError(
                "discord.py n'est pas installé. Les notifications Discord ne sont pas disponibles.\n"
                "Installer avec: pip install discord.py>=2.3.0"
            )
    ...
    def _load_config_from_env(self) -> DiscordConfig:
        ...
        if not bot_token:
            raise ValueError(
                "DISCORD_BOT_TOKEN est requis pour utiliser Discord!\n"
        ...
        if not main_channel:
            raise ValueError(
                "DISCORD_MAIN_CHANNEL_ID est requis!\n"
                "Définir dans .env ou variable environnement"
            )
    ...
    def _validate_config(self):
        ...
        if not self.config.bot_token:
            raise ValueError("Bot token Discord requis")

        if not self.config.main_channel_id:
            raise ValueError("Main channel ID requis")
        ...
        while self.config.auto_reconnect and self.reconnect_attempts < self.config.max_reconnect_attempts:
            try:
                ...
                if self.reconnect_attempts >= self.config.max_reconnect_attempts:
                    ...
                    raise RuntimeError("Impossible de connecter Discord")
    ...
    async def _send_startup_message(self):
        ...
        embed_data = {
            ...
            'fields': [
                {'name': '📊 Mode', 'value': os.environ.get(
                    'TRADING_MODE', 'Unknown'), 'inline': True},
    ...
    async def send_trade_open(self, trade_data: Dict[str, Any]):
        ...
        embed_data = {
            ...
            'fields': [
                ...
                {'name': '📈 Confiance',
                    'value': f"{trade_data.get('confidence', 0):.1%}", 'inline': True}
            ]
        }
    ...
    async def send_trade_close(self, trade_result: Dict[str, Any]):
        ...
        embed_data = {
            ...
            'fields': [
                ...
                {'name': '⏱️ Durée',
                    'value': f"{trade_result.get('duration', 0)} min", 'inline': True},
                {'name': '📈 Entry', 'value': str(
                    trade_result.get('entry_price', 0)), 'inline': True},
                {'name': '📉 Exit', 'value': str(trade_result.get('exit_price', 0)), 'inline': True},
                {'name': '🎯 Raison', 'value': trade_result.get(
                    'exit_reason', 'N/A'), 'inline': True}
            ]
        }
    ...
    async def send_performance_update(self, performance_data: Dict[str, Any]):
        ...
        embed_data = {
            ...
            'fields': [
                {'name': '📊 Trades', 'value': str(
                    performance_data.get('total_trades', 0)), 'inline': True},
                {'name': '📈 Win Rate',
                    'value': f"{performance_data.get('win_rate', 0):.1%}", 'inline': True},
                {'name': '💰 P&L Total',
                    'value': f"${performance_data.get('total_pnl', 0):.2f}", 'inline': True},
                {'name': '📉 Max DD',
                    'value': f"{performance_data.get('max_drawdown', 0):.1%}", 'inline': True},
                {'name': '🎯 Sharpe',
                    'value': f"{performance_data.get('sharpe_ratio', 0):.2f}", 'inline': True},
                {'name': '⚡ Profit Factor',
                    'value': f"{performance_data.get('profit_factor', 0):.2f}", 'inline': True}
            ]
        }
...
def create_discord_notifier(config: Optional[DiscordConfig] = None) -> Optional[DiscordNotifier]:
    """
    Crée un notifier Discord si disponible
    ...
    """
    if not DISCORD_AVAILABLE:
        logger.warning("Discord non disponible - notifications désactivées")
        return None
...
async def test_discord_notifier():
    ...
    if not notify_discord_available():
        logger.warning("Discord non disponible - skip tests")
        return False
