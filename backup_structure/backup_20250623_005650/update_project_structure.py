...
def test_{file_name[:-3]}():
    ...
    logger.info("{{'{class_name}'}} test OK")
    return True
...
def main():
    ...
    logger.info("🚀 MIA_IA_SYSTEM - Automation Mode")
    
    # Load config
    config = get_automation_config()
    
    if not config.automation.enabled:
        logger.error("Automation désactivée dans config")
        return False
    ...
    # Start automation
    try:
        logger.info("Démarrage automation...")
        trader.run_trading_session()
    except KeyboardInterrupt:
        logger.info("🛑 Arrêt automation par utilisateur")
    except Exception as e:
        logger.error("Erreur automation: {e}")
        return False
...
def main():
    ...
    logger.info("📊 MIA_IA_SYSTEM - Data Collection Mode")
    ...
    try:
        ...
        logger.info("🛑 Arrêt collection par utilisateur")
    except Exception as e:
        logger.error("Erreur collection: {e}")
        return False
...
"""
    ...
    def run_update(self, dry_run: bool = False) -> bool:
        ...
        logger.info("🚀 MIA_IA_SYSTEM Structure Updater")
        logger.info("{'='*50}")
        logger.info("Mode: {'🔍 DRY RUN' if dry_run else '🔥 EXECUTION'}")
        logger.info("Projet: {self.project_root}")
        print()
...
def main():
    ...
    try:
        ...
        if success:
            ...
            if args.dry_run:
                logger.info("💡 Relancez sans --dry-run pour appliquer les modifications")
        else:
