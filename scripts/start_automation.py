"""
scripts/start_automation.py

SCRIPT DÉMARRAGE AUTOMATION - OBJECTIF PRIORITÉ 9
Interface simple pour lancer l'automation Battle Navale en différents modes
Point d'entrée unique pour production, test, et collection de données

FONCTIONNALITÉS :
1. start_paper_trading() - Démarrage paper trading sécurisé
2. start_data_collection() - Collection données sans trading
3. start_live_trading() - Trading live avec capitaux réels
4. emergency_stop_all() - Arrêt d'urgence système complet
5. Monitoring intégré et gestion erreurs
6. Configuration dynamique par mode
7. Logging détaillé et alertes

ARCHITECTURE : Production-ready, failsafe, monitoring complet
"""

# === STDLIB ===
import os
import sys
import time
import signal
from core.logger import get_logger
import argparse
import threading
import subprocess
from typing import Dict, List, Optional, Any
from pathlib import Path
from datetime import datetime, timezone
from enum import Enum
import json
import psutil

# === THIRD-PARTY ===
import numpy as np
import pandas as pd

# === LOCAL IMPORTS ===
# Configuration
from config import get_trading_config, get_automation_config

# Core Battle Navale
from core.base_types import (
    MarketData, TradingSignal, SignalType,
    TradeResult, ES_TICK_SIZE, ES_TICK_VALUE
)

# Logger
logger = get_logger(__name__)

# === AUTOMATION ENUMS ===


class AutomationMode(Enum):
    """Modes d'automation"""
    PAPER_TRADING = "paper_trading"      # Paper trading sécurisé
    DATA_COLLECTION = "data_collection"  # Collection données uniquement
    LIVE_TRADING = "live_trading"        # Trading avec capital réel
    BACKTEST = "backtest"               # Backtest historique
    VALIDATION = "validation"           # Validation modèles


class SystemStatus(Enum):
    """États du système"""
    STOPPED = "stopped"
    STARTING = "starting"
    RUNNING = "running"
    STOPPING = "stopping"
    ERROR = "error"
    EMERGENCY_STOP = "emergency_stop"


class AutomationHealth(Enum):
    """Santé de l'automation"""
    HEALTHY = "healthy"
    WARNING = "warning"
    CRITICAL = "critical"
    FAILURE = "failure"

# === AUTOMATION MANAGER ===


class AutomationManager:
    """
    GESTIONNAIRE CENTRAL D'AUTOMATION

    Orchestrateur principal pour tous les modes d'automation :
    - Gestion lifecycle (start/stop/restart)
    - Monitoring temps réel
    - Gestion erreurs et recovery
    - Coordination composants
    - Sécurité et failsafes
    """

    def __init__(self):
        """Initialisation du gestionnaire d'automation"""

        # Configuration
        self.trading_config = get_trading_config()
        self.auto_config = get_automation_config()

        # État système
        self.status = SystemStatus.STOPPED
        self.current_mode: Optional[AutomationMode] = None
        self.health = AutomationHealth.HEALTHY
        self.start_time: Optional[datetime] = None

        # Composants principaux (simulés pour cette phase)
        self.trader = None
        self.snapshotter = None
        self.monitor = None
        self.performance_tracker = None
        self.alert_system = None
        self.data_collector = None
        self.ml_model = None

        # Threads de monitoring
        self.monitoring_thread: Optional[threading.Thread] = None
        self.health_check_thread: Optional[threading.Thread] = None

        # Flags de contrôle
        self.shutdown_requested = False
        self.emergency_stop_triggered = False

        # Paths et logging
        self.log_dir = Path("logs/automation")
        self.log_dir.mkdir(parents=True, exist_ok=True)

        # Setup signal handlers pour arrêt propre
        self._setup_signal_handlers()

        logger.info("AutomationManager initialisé")

    def start_paper_trading(self,
                            duration_hours: Optional[int] = None,
                            max_trades: Optional[int] = None) -> bool:
        """
        Démarrage paper trading sécurisé

        Args:
            duration_hours: Durée maximale en heures (None = illimité)
            max_trades: Nombre maximum de trades (None = illimité)

        Returns:
            True si démarrage réussi, False sinon
        """
        logger.info("=== DÉMARRAGE PAPER TRADING ===")

        if self.status != SystemStatus.STOPPED:
            logger.error(f"Impossible de démarrer - Status actuel: {self.status.value}")
            return False

        try:
            self.status = SystemStatus.STARTING
            self.current_mode = AutomationMode.PAPER_TRADING

            # 1. Initialisation composants paper trading
            self._initialize_paper_trading_components()

            # 2. Validation configuration
            if not self._validate_paper_trading_config():
                self.status = SystemStatus.ERROR
                return False

            # 3. Setup monitoring
            self._setup_monitoring()

            # 4. Configuration limites sécurité
            self._setup_paper_trading_limits(duration_hours, max_trades)

            # 5. Démarrage trader (simulé)
            logger.info("Démarrage trader en mode PAPER...")

            # 6. Démarrage monitoring
            self._start_monitoring_threads()

            # 7. Finalisation
            self.status = SystemStatus.RUNNING
            self.start_time = datetime.now(timezone.utc)

            logger.info("[OK] Paper trading démarré avec succès")
            logger.info(f"Configuration: Durée={duration_hours}h, Max trades={max_trades}")

            return True

        except Exception as e:
            logger.error(f"Erreur démarrage paper trading: {e}")
            self.status = SystemStatus.ERROR
            self.emergency_stop_all()
            return False

    def start_data_collection(self,
                              collection_hours: Optional[int] = None,
                              target_samples: Optional[int] = None) -> bool:
        """
        Démarrage collection de données sans trading

        Args:
            collection_hours: Durée collection en heures
            target_samples: Nombre cible d'échantillons

        Returns:
            True si démarrage réussi, False sinon
        """
        logger.info("=== DÉMARRAGE DATA COLLECTION ===")

        if self.status != SystemStatus.STOPPED:
            logger.error(f"Impossible de démarrer - Status actuel: {self.status.value}")
            return False

        try:
            self.status = SystemStatus.STARTING
            self.current_mode = AutomationMode.DATA_COLLECTION

            # 1. Initialisation composants collection
            self._initialize_data_collection_components()

            # 2. Configuration collection
            collection_config = {
                "duration_hours": collection_hours or 8,
                "target_samples": target_samples or 1000,
                "include_market_data": True,
                "include_signals": True,
                "include_snapshots": True
            }

            # 3. Démarrage collection
            logger.info("Démarrage collection données...")

            # 4. Setup monitoring léger
            self._setup_data_collection_monitoring()

            # 5. Finalisation
            self.status = SystemStatus.RUNNING
            self.start_time = datetime.now(timezone.utc)

            logger.info("[OK] Data collection démarrée avec succès")
            logger.info(f"Configuration: {collection_config}")

            return True

        except Exception as e:
            logger.error(f"Erreur démarrage data collection: {e}")
            self.status = SystemStatus.ERROR
            return False

    def start_live_trading(self,
                           capital_limit: Optional[float] = None,
                           max_position_size: Optional[int] = None) -> bool:
        """
        Démarrage trading live avec capital réel - SÉCURITÉ MAXIMALE

        Args:
            capital_limit: Limite de capital à risquer
            max_position_size: Taille maximale position

        Returns:
            True si démarrage réussi, False sinon
        """
        logger.info("=== DÉMARRAGE LIVE TRADING ===")
        logger.warning("[WARN]  ATTENTION: TRADING AVEC CAPITAL RÉEL [WARN]")

        if self.status != SystemStatus.STOPPED:
            logger.error(f"Impossible de démarrer - Status actuel: {self.status.value}")
            return False

        try:
            self.status = SystemStatus.STARTING
            self.current_mode = AutomationMode.LIVE_TRADING

            # 1. VALIDATIONS SÉCURITÉ CRITIQUES
            if not self._validate_live_trading_safety():
                logger.error("[ALERT] VALIDATION SÉCURITÉ ÉCHEC - ARRÊT")
                self.status = SystemStatus.ERROR
                return False

            # 2. Confirmation utilisateur obligatoire
            if not self._confirm_live_trading():
                logger.info("Live trading annulé par utilisateur")
                self.status = SystemStatus.STOPPED
                return False

            # 3. Initialisation composants live trading
            self._initialize_live_trading_components()

            # 4. Configuration limites strictes
            self._setup_live_trading_limits(capital_limit, max_position_size)

            # 5. Test connexions critiques
            if not self._test_live_connections():
                logger.error("Test connexions live échec")
                self.status = SystemStatus.ERROR
                return False

            # 6. Démarrage avec monitoring renforcé
            self._setup_live_monitoring()

            # 7. Démarrage trader live
            logger.info("[LAUNCH] Démarrage trader en mode LIVE...")

            # 8. Monitoring threads
            self._start_monitoring_threads()

            # 9. Finalisation
            self.status = SystemStatus.RUNNING
            self.start_time = datetime.now(timezone.utc)

            logger.info("[OK] Live trading démarré avec CAPITAL RÉEL")
            logger.warning("🔴 SURVEILLANCE CONTINUE REQUISE")

            return True

        except Exception as e:
            logger.error(f"Erreur critique live trading: {e}")
            self.status = SystemStatus.ERROR
            self.emergency_stop_all()
            return False

    def emergency_stop_all(self) -> bool:
        """
        Arrêt d'urgence complet du système

        Returns:
            True si arrêt réussi, False sinon
        """
        logger.warning("[ALERT] ARRÊT D'URGENCE DÉCLENCHÉ [ALERT]")

        self.emergency_stop_triggered = True
        self.status = SystemStatus.EMERGENCY_STOP

        try:
            # 1. Arrêt immédiat trader
            if self.trader:
                logger.info("Arrêt d'urgence trader...")
                # self.trader.emergency_stop()

            # 2. Fermeture positions ouvertes
            self._close_all_positions_emergency()

            # 3. Arrêt composants
            self._stop_all_components()

            # 4. Arrêt threads
            self._stop_monitoring_threads()

            # 5. Sauvegarde état d'urgence
            self._save_emergency_state()

            # 6. Finalisation
            self.status = SystemStatus.STOPPED
            self.current_mode = None

            logger.info("[OK] Arrêt d'urgence terminé")
            return True

        except Exception as e:
            logger.error(f"Erreur lors arrêt d'urgence: {e}")
            return False

    def get_system_status(self) -> Dict[str, Any]:
        """
        Statut complet du système

        Returns:
            Dictionnaire avec statut détaillé
        """
        runtime = None
        if self.start_time:
            runtime = (datetime.now(timezone.utc) - self.start_time).total_seconds()

        return {
            "status": self.status.value,
            "mode": self.current_mode.value if self.current_mode else None,
            "health": self.health.value,
            "start_time": self.start_time.isoformat() if self.start_time else None,
            "runtime_seconds": runtime,
            "emergency_stop": self.emergency_stop_triggered,
            "components": {
                "trader": self.trader is not None,
                "monitor": self.monitor is not None,
                "data_collector": self.data_collector is not None,
            },
            "system_resources": self._get_system_resources()
        }

    def stop_automation(self) -> bool:
        """
        Arrêt propre de l'automation

        Returns:
            True si arrêt réussi, False sinon
        """
        logger.info("Début arrêt propre automation...")

        if self.status == SystemStatus.STOPPED:
            logger.info("Système déjà arrêté")
            return True

        try:
            self.status = SystemStatus.STOPPING
            self.shutdown_requested = True

            # 1. Arrêt propre trader
            if self.trader:
                logger.info("Arrêt propre trader...")
                # self.trader.stop_trading()

            # 2. Finalisation snapshots
            if self.snapshotter:
                logger.info("Finalisation snapshots...")
                # self.snapshotter.finalize_session()

            # 3. Arrêt monitoring
            self._stop_monitoring_threads()

            # 4. Sauvegarde finale
            self._save_final_state()

            # 5. Finalisation
            self.status = SystemStatus.STOPPED
            self.current_mode = None

            logger.info("[OK] Arrêt propre terminé")
            return True

        except Exception as e:
            logger.error(f"Erreur arrêt propre: {e}")
            return self.emergency_stop_all()

    # === MÉTHODES PRIVÉES ===

    def _setup_signal_handlers(self):
        """Configuration gestionnaires de signaux"""
        def signal_handler(signum, frame):
            logger.warning(f"Signal {signum} reçu - arrêt propre...")
            self.stop_automation()
            sys.exit(0)

        signal.signal(signal.SIGINT, signal_handler)
        signal.signal(signal.SIGTERM, signal_handler)

    def _initialize_paper_trading_components(self):
        """Initialisation composants pour paper trading"""
        logger.info("Initialisation composants paper trading...")

        # Simulation des composants (à remplacer par vrais imports)
        self.trader = "SimulatedTrader"
        self.snapshotter = "SimulatedSnapshotter"
        self.monitor = "SimulatedMonitor"

        logger.info("Composants paper trading initialisés")

    def _initialize_data_collection_components(self):
        """Initialisation composants pour collection données"""
        logger.info("Initialisation composants data collection...")

        # Simulation des composants
        self.data_collector = "SimulatedDataCollector"
        self.snapshotter = "SimulatedSnapshotter"

        logger.info("Composants data collection initialisés")

    def _initialize_live_trading_components(self):
        """Initialisation composants pour live trading"""
        logger.info("Initialisation composants live trading...")

        # Simulation des composants
        self.trader = "LiveTrader"
        self.snapshotter = "LiveSnapshotter"
        self.monitor = "LiveMonitor"
        self.alert_system = "LiveAlertSystem"

        logger.info("Composants live trading initialisés")

    def _validate_paper_trading_config(self) -> bool:
        """Validation configuration paper trading"""
        logger.info("Validation configuration paper trading...")

        # Validations basiques
        if not self.trading_config:
            logger.error("Configuration trading manquante")
            return False

        if not self.auto_config:
            logger.error("Configuration automation manquante")
            return False

        logger.info("[OK] Configuration paper trading validée")
        return True

    def _validate_live_trading_safety(self) -> bool:
        """Validation sécurité critique pour live trading"""
        logger.info("[SEARCH] Validation sécurité live trading...")

        safety_checks = []

        # 1. Configuration présente
        if self.trading_config and self.auto_config:
            safety_checks.append("Configuration OK")
        else:
            logger.error("[ERROR] Configuration manquante")
            return False

        # 2. Limites de risque configurées
        if hasattr(self.auto_config, 'max_daily_loss') and self.auto_config.max_daily_loss > 0:
            safety_checks.append("Limites risque configurées")
        else:
            logger.error("[ERROR] Limites de risque non configurées")
            return False

        # 3. Mode paper validé avant
        # TODO: Vérifier historique paper trading
        safety_checks.append("Paper trading prerequis OK")

        # 4. Validation modèle
        # TODO: Vérifier performance modèle ML
        safety_checks.append("Modèle validé")

        logger.info(f"[OK] Validation sécurité: {len(safety_checks)} checks OK")
        for check in safety_checks:
            logger.info(f"  ✓ {check}")

        return True

    def _confirm_live_trading(self) -> bool:
        """Confirmation utilisateur pour live trading"""
        logger.warning("[WARN]  CONFIRMATION REQUISE POUR LIVE TRADING [WARN]")

        # En production, demander confirmation interactive
        # Pour ce script, on simule une confirmation

        confirmation_text = """
        [ALERT] ATTENTION: TRADING AVEC CAPITAL RÉEL [ALERT]

        Vous êtes sur le point de démarrer le trading automatique
        avec de l'argent réel. Ceci peut entraîner des pertes.

        Confirmez-vous vouloir continuer ? (yes/no):
        """

        # Simulation: en production, utiliser input()
        # response = input(confirmation_text).lower().strip()
        # return response == "yes"

        # Pour cette démo, on simule une confirmation
        logger.warning("Confirmation automatique activée pour démo")
        return True

    def _setup_monitoring(self):
        """Configuration monitoring standard"""
        logger.info("Configuration monitoring...")

        # Setup monitoring components (simulé)
        self.monitor = "StandardMonitor"
        self.performance_tracker = "StandardPerformanceTracker"

        logger.info("Monitoring configuré")

    def _setup_data_collection_monitoring(self):
        """Configuration monitoring pour data collection"""
        logger.info("Configuration monitoring data collection...")

        # Monitoring léger pour collection données
        self.monitor = "DataCollectionMonitor"

        logger.info("Monitoring data collection configuré")

    def _setup_live_monitoring(self):
        """Configuration monitoring renforcé pour live trading"""
        logger.info("Configuration monitoring live renforcé...")

        # Monitoring complet pour live trading
        self.monitor = "LiveMonitorReinforced"
        self.performance_tracker = "LivePerformanceTracker"
        self.alert_system = "LiveAlertSystem"

        logger.info("Monitoring live renforcé configuré")

    def _setup_paper_trading_limits(self, duration_hours, max_trades):
        """Configuration limites paper trading"""
        limits = {
            "duration_hours": duration_hours or 24,
            "max_trades": max_trades or 100,
            "max_position_size": 1,  # 1 contrat max en paper
            "max_daily_loss": 500   # $500 max loss simulé
        }

        logger.info(f"Limites paper trading: {limits}")

    def _setup_live_trading_limits(self, capital_limit, max_position_size):
        """Configuration limites strictes live trading"""
        limits = {
            "capital_limit": capital_limit or 5000,  # $5000 max par défaut
            "max_position_size": max_position_size or 1,  # 1 contrat max
            "max_daily_loss": 200,  # $200 max loss par jour
            "max_daily_trades": 10,  # 10 trades max par jour
            "stop_loss_mandatory": True
        }

        logger.warning(f"[LOCK] Limites STRICTES live trading: {limits}")

    def _test_live_connections(self) -> bool:
        """Test connexions critiques pour live trading"""
        logger.info("Test connexions live...")

        # Test connexions broker, données, etc.
        connections_ok = [
            "Broker connection OK",
            "Market data feed OK",
            "Risk management system OK"
        ]

        for connection in connections_ok:
            logger.info(f"✓ {connection}")

        return True

    def _start_monitoring_threads(self):
        """Démarrage threads de monitoring"""
        logger.info("Démarrage threads monitoring...")

        # Thread monitoring principal
        self.monitoring_thread = threading.Thread(
            target=self._monitoring_loop,
            daemon=True
        )
        self.monitoring_thread.start()

        # Thread health check
        self.health_check_thread = threading.Thread(
            target=self._health_check_loop,
            daemon=True
        )
        self.health_check_thread.start()

        logger.info("Threads monitoring démarrés")

    def _monitoring_loop(self):
        """Boucle principale de monitoring"""
        logger.info("Démarrage boucle monitoring...")

        while not self.shutdown_requested and self.status == SystemStatus.RUNNING:
            try:
                # Monitoring des composants
                self._check_components_health()

                # Monitoring performance
                self._check_performance_metrics()

                # Monitoring ressources système
                self._check_system_resources()

                # Pause
                time.sleep(30)  # Check toutes les 30 secondes

            except Exception as e:
                logger.error(f"Erreur monitoring loop: {e}")
                time.sleep(10)

        logger.info("Arrêt boucle monitoring")

    def _health_check_loop(self):
        """Boucle de vérification santé système"""
        logger.info("Démarrage health check...")

        while not self.shutdown_requested and self.status == SystemStatus.RUNNING:
            try:
                # Health check complet
                self._perform_health_check()

                # Pause plus longue pour health check
                time.sleep(60)  # Check toutes les minutes

            except Exception as e:
                logger.error(f"Erreur health check: {e}")
                time.sleep(30)

        logger.info("Arrêt health check")

    def _check_components_health(self):
        """Vérification santé des composants"""
        # Simulation check composants
        if self.current_mode == AutomationMode.LIVE_TRADING:
            # Checks plus stricts en live
            pass

        # logger.debug("Components health OK")

    def _check_performance_metrics(self):
        """Vérification métriques de performance"""
        # Simulation check performance
        # logger.debug("Performance metrics OK")
        pass

    def _check_system_resources(self):
        """Vérification ressources système"""
        try:
            cpu_percent = psutil.cpu_percent()
            memory = psutil.virtual_memory()

            if cpu_percent > 90:
                logger.warning(f"[WARN] CPU élevé: {cpu_percent}%")
                self.health = AutomationHealth.WARNING

            if memory.percent > 90:
                logger.warning(f"[WARN] Mémoire élevée: {memory.percent}%")
                self.health = AutomationHealth.WARNING

        except Exception as e:
            logger.error(f"Erreur check ressources: {e}")

    def _perform_health_check(self):
        """Health check complet du système"""
        try:
            # Reset health status
            self.health = AutomationHealth.HEALTHY

            # Vérifications selon le mode
            if self.current_mode == AutomationMode.LIVE_TRADING:
                self._live_trading_health_check()
            elif self.current_mode == AutomationMode.PAPER_TRADING:
                self._paper_trading_health_check()
            elif self.current_mode == AutomationMode.DATA_COLLECTION:
                self._data_collection_health_check()

        except Exception as e:
            logger.error(f"Erreur health check: {e}")
            self.health = AutomationHealth.FAILURE

    def _live_trading_health_check(self):
        """Health check spécifique live trading"""
        # Checks critiques pour live trading
        checks_passed = 0
        total_checks = 5

        # 1. Connexion broker
        if self.trader:
            checks_passed += 1

        # 2. Données marché
        checks_passed += 1

        # 3. Système risque
        checks_passed += 1

        # 4. Performance dans limites
        checks_passed += 1

        # 5. Pas de situations d'urgence
        if not self.emergency_stop_triggered:
            checks_passed += 1

        # Évaluation santé
        if checks_passed == total_checks:
            self.health = AutomationHealth.HEALTHY
        elif checks_passed >= 4:
            self.health = AutomationHealth.WARNING
        else:
            self.health = AutomationHealth.CRITICAL
            logger.error(f"[ALERT] SANTÉ CRITIQUE: {checks_passed}/{total_checks} checks OK")

    def _paper_trading_health_check(self):
        """Health check pour paper trading"""
        self.health = AutomationHealth.HEALTHY  # Moins critique

    def _data_collection_health_check(self):
        """Health check pour data collection"""
        self.health = AutomationHealth.HEALTHY  # Moins critique

    def _stop_monitoring_threads(self):
        """Arrêt des threads de monitoring"""
        logger.info("Arrêt threads monitoring...")

        self.shutdown_requested = True

        # Attendre arrêt threads
        if self.monitoring_thread and self.monitoring_thread.is_alive():
            self.monitoring_thread.join(timeout=5)

        if self.health_check_thread and self.health_check_thread.is_alive():
            self.health_check_thread.join(timeout=5)

        logger.info("Threads monitoring arrêtés")

    def _close_all_positions_emergency(self):
        """Fermeture d'urgence toutes positions"""
        logger.warning("[ALERT] Fermeture d'urgence positions...")

        if self.current_mode == AutomationMode.LIVE_TRADING:
            # En live, fermeture réelle des positions
            logger.warning("FERMETURE POSITIONS RÉELLES")
            # TODO: Implémentation fermeture positions

        logger.info("Positions fermées (simulation)")

    def _stop_all_components(self):
        """Arrêt de tous les composants"""
        logger.info("Arrêt composants...")

        # Arrêt simulé des composants
        self.trader = None
        self.snapshotter = None
        self.monitor = None
        self.data_collector = None

        logger.info("Composants arrêtés")

    def _save_emergency_state(self):
        """Sauvegarde état d'urgence"""
        emergency_state = {
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "mode": self.current_mode.value if self.current_mode else None,
            "reason": "emergency_stop_triggered",
            "system_status": self.get_system_status()
        }

        emergency_file = self.log_dir / f"emergency_state_{int(time.time())}.json"

        try:
            with open(emergency_file, 'w') as f:
                json.dump(emergency_state, f, indent=2)

            logger.info(f"État d'urgence sauvegardé: {emergency_file}")

        except Exception as e:
            logger.error(f"Erreur sauvegarde état urgence: {e}")

    def _save_final_state(self):
        """Sauvegarde état final"""
        final_state = {
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "mode": self.current_mode.value if self.current_mode else None,
            "runtime_seconds": (datetime.now(timezone.utc) - self.start_time).total_seconds() if self.start_time else 0,
            "system_status": self.get_system_status()
        }

        final_file = self.log_dir / f"final_state_{int(time.time())}.json"

        try:
            with open(final_file, 'w') as f:
                json.dump(final_state, f, indent=2)

            logger.info(f"État final sauvegardé: {final_file}")

        except Exception as e:
            logger.error(f"Erreur sauvegarde état final: {e}")

    def _get_system_resources(self) -> Dict[str, Any]:
        """Informations ressources système"""
        try:
            return {
                "cpu_percent": psutil.cpu_percent(),
                "memory_percent": psutil.virtual_memory().percent,
                "disk_usage": psutil.disk_usage('/').percent if os.name != 'nt' else psutil.disk_usage('C:').percent,
                "process_count": len(psutil.pids())
            }
        except Exception:
            return {"error": "Unable to get system resources"}

# === GLOBAL AUTOMATION MANAGER ===


# Instance globale du gestionnaire
automation_manager = AutomationManager()

# === FONCTIONS PRINCIPALES ===


def start_paper_trading(duration_hours: Optional[int]
                        = None, max_trades: Optional[int] = None) -> bool:
    """
    Démarrage paper trading sécurisé

    Args:
        duration_hours: Durée en heures (None = illimité)
        max_trades: Nombre max trades (None = illimité)

    Returns:
        True si succès, False sinon
    """
    return automation_manager.start_paper_trading(duration_hours, max_trades)


def start_data_collection(
        collection_hours: Optional[int] = None, target_samples: Optional[int] = None) -> bool:
    """
    Démarrage collection données sans trading

    Args:
        collection_hours: Durée collection en heures
        target_samples: Nombre cible échantillons

    Returns:
        True si succès, False sinon
    """
    return automation_manager.start_data_collection(collection_hours, target_samples)


def start_live_trading(capital_limit: Optional[float] = None,
                       max_position_size: Optional[int] = None) -> bool:
    """
    Démarrage trading live avec capital réel

    Args:
        capital_limit: Limite capital à risquer
        max_position_size: Taille max position

    Returns:
        True si succès, False sinon
    """
    return automation_manager.start_live_trading(capital_limit, max_position_size)


def emergency_stop_all() -> bool:
    """
    Arrêt d'urgence complet du système

    Returns:
        True si succès, False sinon
    """
    return automation_manager.emergency_stop_all()


def get_automation_status() -> Dict[str, Any]:
    """
    Statut actuel de l'automation

    Returns:
        Dictionnaire avec statut détaillé
    """
    return automation_manager.get_system_status()


def stop_automation() -> bool:
    """
    Arrêt propre de l'automation

    Returns:
        True si succès, False sinon
    """
    return automation_manager.stop_automation()

# === CLI INTERFACE ===


def main():
    """Interface en ligne de commande"""

    parser = argparse.ArgumentParser(description="MIA Trading Automation Starter")
    parser.add_argument("mode", choices=["paper", "data", "live", "status", "stop", "emergency"],
                        help="Mode d'automation")

    # Arguments optionnels
    parser.add_argument("--duration", type=int, help="Durée en heures")
    parser.add_argument("--max-trades", type=int, help="Nombre max de trades")
    parser.add_argument("--capital-limit", type=float, help="Limite de capital")
    parser.add_argument("--max-position", type=int, help="Taille max position")
    parser.add_argument("--samples", type=int, help="Nombre cible échantillons")
    parser.add_argument("--verbose", "-v", action="store_true", help="Logging verbose")

    args = parser.parse_args()

    # Configuration logging
    log_level = logging.DEBUG if args.verbose else logging.INFO
    logging.basicConfig(
        level=log_level,
        format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
        handlers=[
            logging.FileHandler(
                f"logs/automation/automation_{datetime.now().strftime('%Y%m%d')}.log"),
            logging.StreamHandler()
        ]
    )

    logger.info("=== MIA TRADING AUTOMATION STARTER ===")

    try:
        # Exécution selon mode
        if args.mode == "paper":
            success = start_paper_trading(args.duration, args.max_trades)
            if success:
                logger.info("Paper trading démarré avec succès")
                # Maintenir le processus actif
                try:
                    while automation_manager.status == SystemStatus.RUNNING:
                        time.sleep(10)
                except KeyboardInterrupt:
                    logger.info("Interruption clavier - arrêt propre...")
                    stop_automation()
            else:
                logger.error("Échec démarrage paper trading")
                sys.exit(1)

        elif args.mode == "data":
            success = start_data_collection(args.duration, args.samples)
            if success:
                logger.info("Data collection démarrée avec succès")
                # Maintenir le processus actif
                try:
                    while automation_manager.status == SystemStatus.RUNNING:
                        time.sleep(10)
                except KeyboardInterrupt:
                    logger.info("Interruption clavier - arrêt propre...")
                    stop_automation()
            else:
                logger.error("Échec démarrage data collection")
                sys.exit(1)

        elif args.mode == "live":
            success = start_live_trading(args.capital_limit, args.max_position)
            if success:
                logger.info("Live trading démarré avec succès")
                logger.info("🔴 SURVEILLANCE CONTINUE REQUISE")
                # Maintenir le processus actif
                try:
                    while automation_manager.status == SystemStatus.RUNNING:
                        time.sleep(10)
                except KeyboardInterrupt:
                    logger.info("Interruption clavier - arrêt propre...")
                    stop_automation()
            else:
                logger.error("Échec démarrage live trading")
                sys.exit(1)

        elif args.mode == "status":
            status = get_automation_status()
            logger.info("[STATS] STATUT AUTOMATION:")
            print(json.dumps(status, indent=2))

        elif args.mode == "stop":
            success = stop_automation()
            if success:
                logger.info("Automation arrêtée proprement")
            else:
                logger.error("Erreur arrêt automation")
                sys.exit(1)

        elif args.mode == "emergency":
            success = emergency_stop_all()
            if success:
                logger.info("[ALERT] ARRÊT D'URGENCE EFFECTUÉ")
            else:
                logger.error("Erreur arrêt d'urgence")
                sys.exit(1)

    except Exception as e:
        logger.error(f"Erreur critique: {e}")
        logger.error("Erreur critique: {e}")
        emergency_stop_all()
        sys.exit(1)

# === TEST FUNCTION ===


def test_automation_starter():
    """Test complet du starter d'automation"""
    logger.info("=== TEST AUTOMATION STARTER ===")

    logger.info("Test 1: Paper Trading")
    success = start_paper_trading(duration_hours=1, max_trades=5)
    logger.info("Paper trading: {'[OK]' if success else '[ERROR]'}")

    if success:
        time.sleep(2)

        status = get_automation_status()
        logger.info("Status: {status['status']}")

        stop_success = stop_automation()
        logger.info("Stop: {'[OK]' if stop_success else '[ERROR]'}")

    logger.info("\nTest 2: Data Collection")
    success = start_data_collection(collection_hours=1, target_samples=100)
    logger.info("Data collection: {'[OK]' if success else '[ERROR]'}")

    if success:
        time.sleep(2)
        stop_success = stop_automation()
        logger.info("Stop: {'[OK]' if stop_success else '[ERROR]'}")

    logger.info("\nTest 3: Emergency Stop")
    emergency_success = emergency_stop_all()
    logger.info("Emergency stop: {'[OK]' if emergency_success else '[ERROR]'}")

    logger.info("=== TEST TERMINÉ ===")


if __name__ == "__main__":
    if len(sys.argv) > 1:
        main()
    else:
        logger.info("Usage: python start_automation.py {paper|data|live|status|stop|emergency}")
        logger.info("Ou lancez test_automation_starter() pour tests")
