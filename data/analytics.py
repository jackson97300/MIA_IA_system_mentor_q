    ...
    def _generate_warnings(self, report: AnalyticsReport) -> List[str]:
        ...
        if report.performance:
            ...
            if perf.consecutive_losses > 5:
                warnings.append(f"[WARN] {perf.consecutive_losses} pertes consécutives détectées")
            
            if perf.max_drawdown > 0.3 * abs(perf.total_pnl):
                warnings.append(f"[WARN] Drawdown critique: {perf.max_drawdown:.2f}")
        
        if report.risk:
            risk = report.risk
            
            if risk.ulcer_index > 10:
                warnings.append(f"[WARN] Ulcer Index élevé: {risk.ulcer_index:.1f}")
        
        if report.total_records < 30:
            warnings.append("[WARN] Échantillon limité - résultats peu fiables")
...
def test_data_analytics():
    ...
    print(f"[OK] Performance analysée: {perf_metrics.total_trades} trades, "
          f"win rate {perf_metrics.win_rate:.1%}")
    
    # Test analyse patterns
    pattern_analysis = analytics.analyze_patterns(test_trades)
    print(f"[OK] Patterns analysés: {len(pattern_analysis.pattern_frequency)} types")
    
    # Test analyse risk avec nouvelles métriques
    risk_analysis = analytics.analyze_risk(test_trades)
    print(f"[OK] Risk analysé: Tail Ratio {risk_analysis.tail_ratio:.2f}, "
          f"Omega Ratio {risk_analysis.omega_ratio:.2f}")
    
    # Test patterns avancés
    if pattern_analysis.time_patterns:
        print(f"[OK] Time patterns analysés: {len(pattern_analysis.time_patterns.get('hourly_stats', {}))} heures")
    
    if pattern_analysis.volatility_patterns:
        print(f"[OK] Volatility patterns analysés: {len(pattern_analysis.volatility_patterns.get('regime_stats', {}))} régimes")
    
    # Test rapport complet
    report = analytics.generate_comprehensive_report()
    print(f"[OK] Rapport généré: {report.report_id}")
    
    # Test sauvegarde
    filepath = analytics.save_report(report, ReportFormat.JSON)
    print(f"[OK] Rapport sauvegardé: {filepath}")
    
    # Test rapport Markdown
    md_filepath = analytics.save_report(report, ReportFormat.MARKDOWN)
    print(f"[OK] Rapport Markdown sauvegardé: {md_filepath}")
