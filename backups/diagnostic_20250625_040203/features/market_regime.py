...
class MarketRegimeDetector:
    ...
    def _analyze_trend_structure(self,
        ...
        trend_strength_value = (structure_score * 0.4 + slope_score * 0.35 + momentum_score_norm * 0.25)
    ...
    def _analyze_es_nq_correlation(self,
        ...
        # Divergence analysis
        if len(es_prices) >= 10:
            es_momentum = (es_prices[-1] - es_prices[-10]) / es_prices[-10] if es_prices[-10] != 0 else 0
            nq_momentum = (nq_prices[-1] - nq_prices[-10]) / nq_prices[-10] if nq_prices[-10] != 0 else 0
        ...
        # Leadership analysis
        if len(es_prices) >= 5:
            es_recent_change = (es_prices[-1] - es_prices[-5]) / es_prices[-5] if es_prices[-5] != 0 else 0
            nq_recent_change = (nq_prices[-1] - nq_prices[-5]) / nq_prices[-5] if nq_prices[-5] != 0 else 0
...
def test_market_regime_detector():
    ...
    for i in range(15):
        micro_price = micro_support + (i % 2) * (micro_resistance - micro_support) + np.random.normal(0, 0.3)
