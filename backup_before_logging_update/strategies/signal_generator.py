...
class SignalGenerator:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # CORRECTION : create_feature_calculator ne prend que l'argument config
        self.feature_calculator = create_feature_calculator(config=config)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info(f"📊 Feature Calculator: {calc_type}")
        ...
        if 'cache_performance' in stats:
            ...
            logger.info(
                f"📊 Cache: Hit rate={cache['hit_rate']:.1%}, Avg={cache['avg_calc_time_ms']:.2f}ms")
